...
export class QuestGenerator {
  ...
  generateCluster(
    ...
    const main = this.generateQuest('main', districtProfile, `${districtId}_main`, 1);
    const sides = [
      this.generateQuest('side', districtProfile, `${districtId}_side_1`, 2),
      this.generateQuest('side', districtProfile, `${districtId}_side_2`, 3),
    ];
    const secret = this.generateQuest('secret', districtProfile, `${districtId}_secret`, 4);
  ...
  private generateQuest(
    ...
    const adjectives = QUEST_ADJECTIVES[districtProfile.theme] ?? QUEST_ADJECTIVES.slums;
    const landmarks = QUEST_LANDMARKS[districtProfile.theme] ?? QUEST_LANDMARKS.slums;
  ...
  private generateRewards(
    ...
    return {
      ...
      alignmentShift: Object.keys(alignmentShift).length > 0 ? alignmentShift : undefined,
    };
  }
