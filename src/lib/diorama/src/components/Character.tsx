...
export function Character({
    ...
    const loadCharacter = async () => {
      try {
        ...
        for (const animPath of animationPaths) {
          const animResult = await SceneLoader.ImportMeshAsync('', '', animPath, scene);
  ...
  }, [scene, modelPath, animationPaths, position, scale, initialAnimation, castShadow, onLoaded]);
...
function findAnimationByPattern(
  ...
  const match = animations.find((anim) => anim.name.toLowerCase().includes(pattern.toLowerCase()));
  return match || null;
}
