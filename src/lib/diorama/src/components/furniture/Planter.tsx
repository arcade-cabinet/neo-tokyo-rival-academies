...
export function Planter({
  ...
  useEffect(() => {
    ...
    if (style === 'box') {
      ...
      // Pot
      const pot = MeshBuilder.CreateSphere(`${id}_pot`, { diameter: diameter, slice: 0.6 }, scene);
      ...
      for (const side of ['front', 'back', 'left', 'right']) {
        let wallW: number, wallH: number, wallD: number, wallX: number, wallZ: number;
    ...
    if (hasPlants) {
      ...
      const baseHeight =
        style === 'urn' ? 0.45 * size : style === 'hanging' ? 0.08 * size : 0.35 * size;
