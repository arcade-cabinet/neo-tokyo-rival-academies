...
const MATERIAL_COLORS: Record<RampMaterial, { base: Color3; metallic: number; roughness: number }> =
  {
...
export function Ramp({
  ...
  useEffect(() => {
    ...
    ramp.position = new Vector3(posX, posY + height / 2, posZ + actualLength / 2);
    ...
    if (sideRails) {
      ...
      for (const side of [-1, 1]) {
        ...
        bottomPost.position = new Vector3(posX + (side * width) / 2, posY + 0.5, posZ);
