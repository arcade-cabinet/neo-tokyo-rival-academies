  ...
  useEffect(() => {
    ...
    if (style === 'standard') {
      ...
      for (let i = 0; i < segments; i++) {
        ...
        segment.position = new Vector3(posX, posY + (y1 + y2) / 2, posZ + (z1 + z2) / 2);
      ...
      // Simple flat awning
      const canopy = MeshBuilder.CreateBox(`${id}_canopy`, { width, height: 0.03, depth }, scene);
      ...
      // Dome/bubble awning
      const dome = MeshBuilder.CreateSphere(`${id}_dome`, { diameter: width, slice: 0.5 }, scene);
      ...
      tradMat.albedoColor = fabric === 'bamboo' ? new Color3(0.6, 0.5, 0.35) : primaryColor;
      tradMat.metallic = 0;
      ...
      if (fabric === 'bamboo') {
        ...
        const panel = MeshBuilder.CreateBox(`${id}_panel`, { width, height: 0.01, depth }, scene);
      ...
      const canopy = MeshBuilder.CreateBox(`${id}_canopy`, { width, height: 0.02, depth }, scene);
      canopy.position = new Vector3(posX, posY, posZ + depth / 2);
      canopy.rotation.x = dropAngle;
      canopy.rotation.y = rotation;
      canopy.material = fabricMat;
      ...
      for (const side of [-1, 1]) {
        ...
        // Elbow joint
        const elbow = MeshBuilder.CreateSphere(`${id}_elbow_${side}`, { diameter: 0.08 }, scene);
