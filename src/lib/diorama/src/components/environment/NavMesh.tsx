...
export function NavMesh({
  ...
  useEffect(() => {
    ...
    const controller: NavMeshController = {
      findPath: (start: Vector3, end: Vector3): Vector3[] => {
        ...
        if (path.length > 0) {
          return [start, ...path.map((i) => waypointsRef.current[i].position), end];
        }
    ...
    function astar(startIdx: number, endIdx: number): number[] {
      ...
      wps[startIdx].hCost = Vector3.Distance(wps[startIdx].position, wps[endIdx].position);
      ...
      while (openSet.size > 0) {
        ...
        for (const neighborIdx of current.neighbors) {
          ...
          const tentativeG =
            current.gCost + Vector3.Distance(current.position, neighbor.position) * cost;

          if (tentativeG < neighbor.gCost) {
            neighbor.parent = currentIdx;
            neighbor.gCost = tentativeG;
            neighbor.hCost = Vector3.Distance(neighbor.position, wps[endIdx].position);
  ...
  }, [scene, id, areas, cellSize, debug, debugColor, waypointColor, connectionColor, onReady]);
