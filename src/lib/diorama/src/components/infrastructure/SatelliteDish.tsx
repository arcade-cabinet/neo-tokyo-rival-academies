...
export function SatelliteDish({
  ...
  useEffect(() => {
    ...
    const brokenAzimuth = isBroken && rng ? azimuthRad + (rng.next() - 0.5) * 0.3 : azimuthRad;
    ...
    const feedX =
      posX +
      Math.sin(brokenAzimuth) * (dishDistFromMount + feedDistance) * Math.cos(brokenElevation);
    const feedY = dishCenterY + (dishDistFromMount + feedDistance) * Math.sin(brokenElevation);
    const feedZ =
      posZ +
      Math.cos(brokenAzimuth) * (dishDistFromMount + feedDistance) * Math.cos(brokenElevation);
    ...
    if (type === 'commercial') {
      ...
      for (let i = 0; i < armCount; i++) {
        ...
        const armStart = new Vector3(
          dish.position.x +
            Math.cos(armAngle) * (actualDiameter * 0.35) * Math.cos(brokenElevation),
          dish.position.y,
    ...
    if (condition === 'pristine' || condition === 'weathered') {
      ...
      label.position = new Vector3(
        dish.position.x -
          Math.sin(brokenAzimuth) * (actualDiameter * 0.3) * Math.cos(brokenElevation - 0.2),
        dish.position.y - actualDiameter * 0.15,
        dish.position.z -
          Math.cos(brokenAzimuth) * (actualDiameter * 0.3) * Math.cos(brokenElevation - 0.2)
      );
