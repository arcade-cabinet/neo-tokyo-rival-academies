  ...
  useEffect(() => {
    ...
    if (type === 'cylindrical') {
      ...
      for (let l = 0; l < legCount; l++) {
        ...
        leg.position = new Vector3(posX + legX, posY + (diameter / 4 + 0.25), posZ + legZ);
    ...
    if (content === 'fuel' || content === 'chemical') {
      ...
      labelMat.albedoColor =
        content === 'fuel' ? new Color3(0.9, 0.1, 0.1) : new Color3(0.9, 0.7, 0.1);
      labelMat.metallic = 0;
      labelMat.roughness = 0.6;

      const label = MeshBuilder.CreatePlane(`${id}_label`, { width: 0.5, height: 0.5 }, scene);
