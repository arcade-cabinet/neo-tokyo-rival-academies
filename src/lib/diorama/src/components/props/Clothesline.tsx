...
export function Clothesline({
  ...
  useEffect(() => {
    ...
    ropeMat.albedoColor =
      type === 'makeshift' ? new Color3(0.4, 0.35, 0.3) : new Color3(0.85, 0.85, 0.8);
    ropeMat.metallic = 0;
    ...
    for (let s = 0; s < segments; s++) {
      ...
      const segLength = Math.sqrt((x2 - x1) ** 2 + (sag2 - sag1) ** 2 + (z2 - z1) ** 2);
      ...
      segment.position = new Vector3((x1 + x2) / 2, posY - (sag1 + sag2) / 2, (z1 + z2) / 2);
    ...
    for (let i = 0; i < itemCount; i++) {
      ...
      const colorIdx = rng ? Math.floor(rng.next() * clothColors.length) : i % clothColors.length;
