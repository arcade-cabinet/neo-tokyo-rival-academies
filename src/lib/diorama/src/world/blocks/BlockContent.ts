 ...
 */
export function getBlockContent(definition: BlockDefinition): BlockContentDefinition | undefined {
...
export function calculateSpawnPositions(
  ...
  const generatePosition = (index: number): Vector3 => {
    ...
    return new Vector3(blockPosition.x + x, blockPosition.y + yOffset, blockPosition.z + z);
  };
  ...
  for (let i = 0; i < count; i++) {
    ...
    do {
      ...
      const tooClose = positions.some((existing) => Vector3.Distance(pos, existing) < spacing);
