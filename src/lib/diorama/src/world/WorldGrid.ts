...
export type CellType = 'building' | 'street' | 'plaza' | 'bridge' | 'elevator' | 'park' | 'alley';
...
export class WorldGrid {
  ...
  private determineStratum(gridX: number, gridZ: number): StratumId {
    ...
    const distFromCenter = Math.sqrt((gridX - centerX) ** 2 + (gridZ - centerZ) ** 2);
   ...
   */
  private determineCellType(gridX: number, gridZ: number, rng: () => number): CellType {
