...
export function getMaterialTexturePath(material: MaterialName, map: PBRMapType): string {
  return `${TEXTURE_BASE_PATH}/materials/${material}/${material}_1K-JPG_${map}.jpg`;
}

/** Get runtime URL for a decal texture */
export function getDecalTexturePath(decal: DecalName, map: PBRMapType | 'Opacity'): string {
...
export function getAllDecalTextureSets(): Record<DecalName, DecalTextureSet> {
  return Object.fromEntries(DECAL_NAMES.map((name) => [name, getDecalTextureSet(name)])) as Record<
    DecalName,
    DecalTextureSet
  >;
}
