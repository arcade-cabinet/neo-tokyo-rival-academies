  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.minimap}>
         ...
         {playerPos && <div style={{position: 'absolute', bottom: 5, right: 5, color: '#0ff', fontSize: '10px'}}>
      ...
      {currentDialogue && (
        <div className={styles.dialogueBox}>
          ...
          <div style={{alignSelf: 'flex-end', fontSize: '0.8rem', color: '#aaa', marginTop: '10px'}}>TAP TO CONTINUE</div>
        </div>
      )}
      ...
      <div className={styles.controls}>
        ...
        <div className={styles.dpad}>
            {/* Left */}
            <div className={styles.dpadBtn} style={{top: '50px', left: '0'}}
                 onTouchStart={handleTouch('left', true)} onTouchEnd={handleTouch('left', false)}>
                 ←
            </div>
            {/* Right */}
            <div className={styles.dpadBtn} style={{top: '50px', right: '0'}}
                 onTouchStart={handleTouch('right', true)} onTouchEnd={handleTouch('right', false)}>
                 →
            </div>
            {/* Down (Slide) */}
            <div className={styles.dpadBtn} style={{bottom: '0', left: '50px'}}
                 onTouchStart={handleTouch('slide', true)} onTouchEnd={handleTouch('slide', false)}>
                 ↓
            </div>
        ...
        <div style={{display: 'flex', alignItems: 'flex-end'}}>
             <div className={styles.actionBtn} style={{background: 'rgba(0, 255, 255, 0.2)', borderColor: '#0ff'}}
                  onTouchStart={handleTouch('attack', true)} onTouchEnd={handleTouch('attack', false)}>
                 ATK
             </div>
             <div className={styles.actionBtn} style={{background: 'rgba(0, 255, 0, 0.2)', borderColor: '#0f0', marginBottom: '40px'}}
                  onTouchStart={handleTouch('jump', true)} onTouchEnd={handleTouch('jump', false)}>
                 JUMP
             </div>
             <div className={styles.actionBtn} style={{background: 'rgba(255, 255, 0, 0.2)', borderColor: '#ff0'}}
                  onTouchStart={handleTouch('run', true)} onTouchEnd={handleTouch('run', false)}>
                 RUN
             </div>
        </div>
      </div>
