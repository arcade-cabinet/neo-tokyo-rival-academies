...
export type BoatType = "rowboat" | "motorboat" | "kayak" | "canoe" | "dinghy" | "sampan";
...
export function Boat({
	...
	useEffect(() => {
		...
		switch (type) {
			case "rowboat":
			case "dinghy":
				hullMat.albedoColor = new Color3(0.45, 0.35, 0.2).scale(conditionFactor);
				hullMat.metallic = 0;
			...
			case "canoe":
				...
				break;
			case "sampan":
				hullMat.albedoColor = new Color3(0.35, 0.28, 0.18).scale(conditionFactor);
				hullMat.metallic = 0;
				hullMat.roughness = 0.9;
				break;
		}

		const width = type === "kayak" ? length * 0.15 : type === "canoe" ? length * 0.2 : length * 0.35;
		const depth = type === "kayak" || type === "canoe" ? length * 0.08 : length * 0.15;
		...
		if (type === "rowboat" || type === "dinghy") {
			...
			const bow = MeshBuilder.CreateCylinder(
				`${id}_bow`,
				{ height: depth, diameterTop: 0.05, diameterBottom: width, tessellation: 3 },
				scene
			);
			...
			canopyMat.albedoColor = new Color3(0.6, 0.55, 0.45).scale(conditionFactor);
		...
		// Oars
		if (hasOars && state !== "overturned" && (type === "rowboat" || type === "canoe" || type === "dinghy")) {
	...
	}, [scene, id, posX, posY, posZ, type, state, length, hasOars, hasMotor, condition, rotation, seed]);
