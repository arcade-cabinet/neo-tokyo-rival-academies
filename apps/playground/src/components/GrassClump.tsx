...
export function GrassClump({
	...
	useEffect(() => {
		...
		switch (type) {
			case "wild":
				grassMat.albedoColor = new Color3(
					0.2 + brownMix * 0.3,
					0.35 + greenBase,
					0.1
				);
		...
		if (type === "moss") {
			...
			// Grass blades
			const bladeCount = Math.floor((type === "lawn" ? 15 : type === "tall" || type === "reeds" ? 8 : 12) * density);
			const bladeHeight = type === "tall" ? height * 2 : type === "reeds" ? height * 3 : type === "lawn" ? height * 0.5 : height;
