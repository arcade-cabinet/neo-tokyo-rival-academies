...
export type CanalType = "concrete" | "stone" | "natural" | "industrial" | "venetian";
...
export function Canal({
	...
	useEffect(() => {
		...
		if (type === "concrete") {
			...
			floorMat.albedoColor = new Color3(0.5, 0.48, 0.45).scale(conditionFactor * 0.9);
			floorMat.metallic = 0;
			floorMat.roughness = 0.85;
			walkwayMat.albedoColor = new Color3(0.65, 0.63, 0.6).scale(conditionFactor);
			...
			floorMat.albedoColor = new Color3(0.4, 0.38, 0.35).scale(conditionFactor * 0.9);
			floorMat.metallic = 0;
			floorMat.roughness = 0.95;
			walkwayMat.albedoColor = new Color3(0.5, 0.48, 0.45).scale(conditionFactor);
			walkwayMat.metallic = 0;
			...
			walkwayMat.albedoColor = new Color3(0.4, 0.35, 0.28).scale(conditionFactor);
			walkwayMat.metallic = 0;
			...
			floorMat.albedoColor = new Color3(0.35, 0.37, 0.4).scale(conditionFactor * 0.9);
			floorMat.metallic = 0.2;
			floorMat.roughness = 0.75;
			walkwayMat.albedoColor = new Color3(0.45, 0.47, 0.5).scale(conditionFactor);
			...
			floorMat.albedoColor = new Color3(0.45, 0.4, 0.35).scale(conditionFactor * 0.9);
			floorMat.metallic = 0;
			floorMat.roughness = 0.9;
			walkwayMat.albedoColor = new Color3(0.7, 0.65, 0.55).scale(conditionFactor);
			walkwayMat.metallic = 0;
		...
		if (hasWalkways) {
			...
			for (const side of [-1, 1]) {
				...
				walkway.position = new Vector3(
					posX + Math.cos(rotation) * (side * (width / 2 + wallThickness + walkwayWidth / 2)),
					posY + 0.075,
					posZ - Math.sin(rotation) * (side * (width / 2 + wallThickness + walkwayWidth / 2))
				);
				...
				curb.position = new Vector3(
					posX + Math.cos(rotation) * (side * (width / 2 + wallThickness + 0.05)),
					posY + 0.1,
					posZ - Math.sin(rotation) * (side * (width / 2 + wallThickness + 0.05))
				);
		...
		if (hasMooring) {
			...
			mooringMat.albedoColor = new Color3(0.3, 0.32, 0.35).scale(conditionFactor);
			...
			for (let r = 0; r < ringCount; r++) {
				...
				for (const side of [-1, 1]) {
					...
					mount.position = new Vector3(
						posX + Math.cos(rotation) * (side * (width / 2 + wallThickness / 2)) - Math.sin(rotation) * rz,
						posY + 0.075,
						posZ - Math.sin(rotation) * (side * (width / 2 + wallThickness / 2)) - Math.cos(rotation) * rz
					);
					...
					ring.position = new Vector3(
						posX + Math.cos(rotation) * (side * (width / 2 + wallThickness + 0.05)) - Math.sin(rotation) * rz,
						posY + 0.15,
						posZ - Math.sin(rotation) * (side * (width / 2 + wallThickness + 0.05)) - Math.cos(rotation) * rz
					);
		...
		if (conditionFactor < 0.7 && type !== "natural") {
			...
			for (let s = 0; s < stainCount; s++) {
				const sz = (rng ? (rng.next() - 0.5) : (s / stainCount - 0.5)) * (length - 2);
				...
				for (const side of [-1, 1]) {
					...
					stain.position = new Vector3(
						posX + Math.cos(rotation) * (side * (width / 2 + 0.01)) - Math.sin(rotation) * sz,
						posY - depth + waterHeight + 0.15,
						posZ - Math.sin(rotation) * (side * (width / 2 + 0.01)) - Math.cos(rotation) * sz
					);
	...
	}, [scene, id, posX, posY, posZ, type, length, width, depth, hasWalkways, hasMooring, waterLevel, condition, rotation, seed]);
