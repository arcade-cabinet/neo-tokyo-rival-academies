...
const CONDITION_FACTORS: Record<ConditionType, { rust: number; roughness: number; clarity: number }> = {
...
export function Skylight({
	...
	useEffect(() => {
		...
		if (type === "flat") {
			...
			for (const side of ["front", "back", "left", "right"]) {
				const isHorizontal = side === "front" || side === "back";
				const topFrameWidth = isHorizontal ? width + frameThickness * 2 : frameThickness;
				const topFrameDepth = isHorizontal ? frameThickness : depth;

				const topFrame = MeshBuilder.CreateBox(
					`${id}_topframe_${side}`,
					{ width: topFrameWidth, height: frameThickness, depth: topFrameDepth },
					scene
				);
				...
				topFrame.position = new Vector3(frameX, posY + curbHeight + frameThickness / 2, frameZ);
			...
			if (!isBroken) {
				...
				const glass = MeshBuilder.CreateBox(
					`${id}_glass`,
					{ width: width - frameThickness, height: 0.01, depth: depth - frameThickness },
					scene
				);
				glass.position = new Vector3(
					posX,
					posY + curbHeight + frameThickness + (isOpen ? Math.sin(openAngle) * depth / 4 : 0),
			...
			if (!isBroken) {
				...
				const flange = MeshBuilder.CreateTorus(
					`${id}_flange`,
					{ diameter: width - frameThickness, thickness: frameThickness * 0.8, tessellation: 32 },
					scene
				);
			...
			if (!isBroken) {
				...
				for (let face = 0; face < 4; face++) {
					...
					const panelLength = Math.sqrt(Math.pow(faceWidth / 2, 2) + Math.pow(pyramidHeight, 2));
					const panelAngle = Math.atan2(pyramidHeight, faceDepth / 2);

					const panel = MeshBuilder.CreateBox(
						`${id}_panel_${face}`,
						{ width: faceWidth * 0.92, height: 0.008, depth: panelLength * 0.9 },
						scene
					);
					...
					const edge = MeshBuilder.CreateBox(
						`${id}_edge_${face}`,
						{ width: frameThickness * 0.8, height: frameThickness * 0.8, depth: panelLength },
						scene
					);
				...
				for (let corner = 0; corner < 4; corner++) {
					...
					const cornerDist = Math.sqrt(Math.pow(width / 2, 2) + Math.pow(depth / 2, 2));
					const ridgeLength = Math.sqrt(Math.pow(cornerDist, 2) + Math.pow(pyramidHeight, 2));
			...
			const cap = MeshBuilder.CreateCylinder(
				`${id}_cap`,
				{ height: 0.04, diameterTop: 0.02, diameterBottom: 0.06, tessellation: 8 },
				scene
			);
		...
		if (isBroken && rng) {
			...
			for (let i = 0; i < shardCount; i++) {
				...
				shard.position = new Vector3(
					shardX,
					posY + (type === "flat" ? 0.12 : type === "domed" ? 0.1 : 0.08) + rng.next() * 0.02,
					shardZ
				);
		...
		if (conditionFactor.clarity < 0.7 && rng && !isBroken) {
			...
			for (let i = 0; i < dirtCount; i++) {
				...
				dirt.position = new Vector3(
					...
					posY + (type === "domed" ? width * 0.2 : type === "pyramid" ? width * 0.25 : 0.14),
	...
	}, [scene, id, posX, posY, posZ, type, width, depth, isOpen, isBroken, condition, rotation, seed]);
