...
export function Carcass({
	...
	useEffect(() => {
		...
		if (type === "motorcycle") {
			...
			// Cabin (upper)
			const cabinLength = type === "truck" ? length * 0.4 : type === "bus" ? length * 0.9 : length * 0.6;
			...
			if (hasWheels) {
				...
				const wheelPositions = type === "bus"
					? [[-length * 0.4, -width * 0.5], [-length * 0.4, width * 0.5], [length * 0.35, -width * 0.5], [length * 0.35, width * 0.5]]
					: [[-length * 0.35, -width * 0.5], [-length * 0.35, width * 0.5], [length * 0.35, -width * 0.5], [length * 0.35, width * 0.5]];
			...
			if (type === "truck") {
				...
				for (const side of [-1, 1]) {
					...
					bedSide.position = new Vector3(
						posX + Math.cos(rotation) * (side * width * 0.47) + Math.sin(rotation) * (length * 0.2),
						baseY + bodyHeight + 0.2,
						posZ - Math.sin(rotation) * (side * width * 0.47) + Math.cos(rotation) * (length * 0.2)
					);
	...
	}, [scene, id, posX, posY, posZ, type, state, hasWheels, missingPanels, condition, rotation, seed]);
