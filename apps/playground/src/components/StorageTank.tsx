...
export type StorageTankType = "cylindrical" | "spherical" | "rectangular" | "silo" | "pressurized";
export type StorageTankContent = "fuel" | "water" | "chemical" | "gas" | "empty";
...
export function StorageTank({
	...
	useEffect(() => {
		...
		switch (content) {
			case "fuel":
				tankMat.albedoColor = new Color3(0.85, 0.85, 0.8).scale(conditionFactor);
				break;
			case "water":
			...
			case "chemical":
				...
				break;
			case "gas":
				tankMat.albedoColor = new Color3(0.6, 0.62, 0.65).scale(conditionFactor);
				break;
			default:
				tankMat.albedoColor = new Color3(0.5, 0.52, 0.55).scale(conditionFactor);
		}
		...
		if (type === "cylindrical") {
			...
			baseRingMat.albedoColor = new Color3(0.4, 0.42, 0.45).scale(conditionFactor);
			...
			const cone = MeshBuilder.CreateCylinder(
				`${id}_cone`,
				{ height: height * 0.3, diameterBottom: diameter, diameterTop: 0.2, tessellation: 24 },
				scene
			);
			...
			saddleMat.albedoColor = new Color3(0.5, 0.52, 0.55).scale(conditionFactor);
		...
		if (hasLadder && type !== "pressurized") {
			...
			ladderMat.albedoColor = new Color3(0.5, 0.52, 0.55).scale(conditionFactor);
			...
			for (const side of [-0.15, 0.15]) {
				...
				rail.position = new Vector3(
					posX + Math.cos(rotation) * (diameter / 2 + 0.1) + Math.sin(rotation) * side,
					posY + ladderHeight / 2,
					posZ - Math.sin(rotation) * (diameter / 2 + 0.1) + Math.cos(rotation) * side
				);
		...
		if (content === "fuel" || content === "chemical") {
			...
			labelMat.albedoColor = content === "fuel" ? new Color3(0.9, 0.1, 0.1) : new Color3(0.9, 0.7, 0.1);
			labelMat.metallic = 0;
	...
	}, [scene, id, posX, posY, posZ, type, content, diameter, height, hasLadder, hasPiping, fillLevel, condition, rotation, seed]);
