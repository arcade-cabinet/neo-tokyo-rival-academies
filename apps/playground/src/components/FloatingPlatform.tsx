...
export function FloatingPlatform({
	...
	useEffect(() => {
		...
		if (type === "dock") {
			// Wooden dock
			deckMat.albedoColor = new Color3(0.45 * conditionFactor, 0.35 * conditionFactor, 0.2 * conditionFactor);
			deckMat.metallic = 0;
			...
			for (let b = 0; b < beamCount; b++) {
				const beamX = (b - (beamCount - 1) / 2) * (width / (beamCount - 1) * 0.8);
			...
			for (let p = 0; p < plankCount; p++) {
				...
				const px = (rng ? (rng.next() - 0.5) : (p / plankCount - 0.5)) * width * 0.8;
			...
			for (let b = 0; b < barrelCount; b++) {
				const bx = ((b % 2) - 0.5) * (width - 0.3) + (rng ? (rng.next() - 0.5) * 0.2 : 0);
				const bz = (Math.floor(b / 2) / (barrelCount / 2) - 0.5) * (depth - 0.4);
			...
			for (const side of [-1, 1]) {
				...
				for (let p = 0; p < postCount; p++) {
					...
					post.position = new Vector3(
						posX + Math.cos(rotation) * (side * (width / 2 - 0.1)) - Math.sin(rotation) * postZ,
						posY + 0.2 + railHeight / 2,
						posZ - Math.sin(rotation) * (side * (width / 2 - 0.1)) - Math.cos(rotation) * postZ
					);
	...
	}, [scene, id, posX, posY, posZ, type, width, depth, hasRailings, hasMooring, condition, rotation, seed]);
