...
export type HouseboatType = "traditional" | "modern" | "shanty" | "luxury" | "converted";
...
export function Houseboat({
	...
	useEffect(() => {
		...
		if (type === "traditional") {
			...
			hullMat.albedoColor = new Color3(0.3, 0.28, 0.25).scale(conditionFactor * 0.7);
			hullMat.metallic = 0.3;
			hullMat.roughness = 0.9;
			wallMat.albedoColor = new Color3(0.4, 0.35, 0.3).scale(conditionFactor * 0.8);
			wallMat.metallic = 0.2;
			wallMat.roughness = 0.85;
			roofMat.albedoColor = new Color3(0.35, 0.32, 0.3).scale(conditionFactor * 0.7);
			...
			for (let w = 0; w < windowCount; w++) {
				...
				for (const side of [-1, 1]) {
					...
					window.position = new Vector3(
						posX + Math.cos(rotation) * (side * cabinWidth / 2) - Math.sin(rotation) * wz,
						cabinY,
						posZ - Math.sin(rotation) * (side * cabinWidth / 2) - Math.cos(rotation) * wz
					);
		...
		if (hasDeck) {
			...
			for (const side of [-1, 1]) {
				...
				railing.position = new Vector3(
					posX + Math.cos(rotation) * (side * (width / 2 - 0.15)) - Math.sin(rotation) * deckZ,
					posY + hullHeight + 0.4,
					posZ - Math.sin(rotation) * (side * (width / 2 - 0.15)) - Math.cos(rotation) * deckZ
				);
	...
	}, [scene, id, posX, posY, posZ, type, length, width, stories, hasDeck, hasPlanters, condition, rotation, seed]);
