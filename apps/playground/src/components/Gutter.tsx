...
const GUTTER_PROFILES: Record<GutterType, { width: number; depth: number; thickness: number }> = {
...
const CONDITION_FACTORS: Record<ConditionType, { rust: number; roughness: number; alpha: number }> = {
...
export function Gutter({
	...
	useEffect(() => {
		...
		if (type === "half_round") {
			...
			const back = MeshBuilder.CreateBox(
				`${id}_back`,
				{ width: length, height: profile.depth * 0.7, depth: profile.thickness },
				scene
			);
			...
			const lip = MeshBuilder.CreateBox(
				`${id}_lip`,
				{ width: length, height: profile.thickness * 2, depth: profile.thickness * 3 },
				scene
			);
			...
			for (const side of [-1, 1]) {
				...
				sideWall.position = new Vector3(
					posX + Math.sin(rotation) * (side * (profile.width / 2 - profile.thickness / 2)),
					posY + profile.depth / 2,
					posZ + Math.cos(rotation) * (side * (profile.width / 2 - profile.thickness / 2))
				);
			...
			for (let i = 0; i < bracketCount; i++) {
				...
				scroll.position = new Vector3(
					posX + Math.cos(rotation) * bracketPos - Math.sin(rotation) * (profile.width / 2 + 0.015),
					posY - 0.06,
					posZ - Math.sin(rotation) * bracketPos - Math.cos(rotation) * (profile.width / 2 + 0.015)
				);
		...
		if (hasDownspout) {
			...
			const elbowTop = MeshBuilder.CreateTorus(
				`${id}_elbow_top`,
				{ diameter: downspoutSize * 2, thickness: downspoutSize / 2, tessellation: 16, arc: 0.25 },
				scene
			);
			...
			for (let i = 0; i < strapCount; i++) {
				const strapY = posY - 0.2 - (i / (strapCount - 1)) * (downspoutLength - 0.2);

				const strap = MeshBuilder.CreateTorus(
					`${id}_strap_${i}`,
					{ diameter: downspoutSize * 1.3, thickness: 0.008, tessellation: 16, arc: 0.75 },
					scene
				);
	...
	}, [scene, id, posX, posY, posZ, type, length, hasDownspout, isLeaking, condition, rotation, seed]);
