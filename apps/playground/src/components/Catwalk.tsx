...
export function Catwalk({
			...
			for (let p = 0; p < poleCount; p++) {
				...
				for (const side of [-1, 1]) {
					...
					pole.position = new Vector3(
						posX + Math.cos(rotation) * poleX + Math.sin(rotation) * (side * width / 2),
						posY - 0.25,
						posZ - Math.sin(rotation) * poleX + Math.cos(rotation) * (side * width / 2)
					);
			...
			for (const side of sides) {
				...
				for (let p = 0; p < postCount; p++) {
					...
					post.position = new Vector3(
						posX + Math.cos(rotation) * postX + Math.sin(rotation) * (side * (width / 2 + 0.02)),
						posY + railHeight / 2,
						posZ - Math.sin(rotation) * postX + Math.cos(rotation) * (side * (width / 2 + 0.02))
					);
	...
	}, [scene, id, posX, posY, posZ, style, length, width, railing, rotation, rust, hasSupports, seed]);
