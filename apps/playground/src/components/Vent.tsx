	...
	useEffect(() => {
		...
		if (type === "turbine") {
			...
			const base = MeshBuilder.CreateCylinder(
				`${id}_base`,
				{ height: baseHeight, diameter: turbineDiameter * 0.7, diameterTop: turbineDiameter * 0.85 },
				scene
			);
			...
			for (let f = 0; f < finCount; f++) {
				...
				const fin = MeshBuilder.CreateBox(
					`${id}_fin_${f}`,
					{ width: 0.02, height: turbineHeight * 0.5, depth: turbineDiameter * 0.35 },
					scene
				);
			...
			const cap = MeshBuilder.CreateCylinder(
				`${id}_cap`,
				{ height: capHeight, diameterTop: capDiameter * 0.3, diameterBottom: capDiameter },
				scene
			);
			...
			for (const side of [-1, 1]) {
				const capPanel = MeshBuilder.CreateBox(
					`${id}_cap_${side}`,
					{ width: ridgeLength, height: ridgeWidth / Math.cos(capAngle) / 2, depth: 0.02 },
					scene
				);
