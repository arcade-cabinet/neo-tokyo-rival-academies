...
export function Anchor({
	...
	useEffect(() => {
		...
		if (type === "traditional") {
			...
			const ring = MeshBuilder.CreateTorus(
				`${id}_ring`,
				{ diameter: baseSize * 0.15, thickness: baseSize * 0.03, tessellation: 16 },
				scene
			);
			ring.position = new Vector3(posX, posY + baseSize + baseSize * 0.08, posZ);
			...
			for (const side of [-1, 1]) {
				...
				const fluke = MeshBuilder.CreateCylinder(
					`${id}_fluke_${side}`,
					{ height: baseSize * 0.2, diameterTop: 0, diameterBottom: baseSize * 0.15, tessellation: 3 },
					scene
				);
			...
			const shank = MeshBuilder.CreateBox(
				`${id}_shank`,
				{ width: baseSize * 0.08, height: baseSize * 0.8, depth: baseSize * 0.04 },
				scene
			);
			...
			const ring = MeshBuilder.CreateTorus(
				`${id}_ring`,
				{ diameter: baseSize * 0.12, thickness: baseSize * 0.02, tessellation: 16 },
				scene
			);
			...
			for (const side of [-1, 1]) {
				const fluke = MeshBuilder.CreateBox(
					`${id}_fluke_${side}`,
					{ width: baseSize * 0.35, height: baseSize * 0.02, depth: baseSize * 0.25 },
					scene
				);
				fluke.position = new Vector3(
			...
			const cap = MeshBuilder.CreateCylinder(
				`${id}_cap`,
				{ height: baseSize * 0.15, diameterTop: baseSize * 0.6, diameterBottom: baseSize * 0.3, tessellation: 16 },
				scene
			);
			...
			const ring = MeshBuilder.CreateTorus(
				`${id}_ring`,
				{ diameter: baseSize * 0.18, thickness: baseSize * 0.03, tessellation: 16 },
				scene
			);
		...
		if (hasChain) {
			...
			for (let l = 0; l < linkCount; l++) {
				const link = MeshBuilder.CreateTorus(
					`${id}_link_${l}`,
					{ diameter: baseSize * 0.08, thickness: baseSize * 0.015, tessellation: 8 },
					scene
				);
	...
	}, [scene, id, posX, posY, posZ, type, state, size, hasChain, chainLength, condition, rotation, seed]);
