...
export function Ladder({
	...
	useEffect(() => {
		...
		if (material === "metal") {
			railMat.albedoColor = new Color3(0.35 * wearFactor, 0.35 * wearFactor, 0.38 * wearFactor);
			railMat.metallic = 0.85;
			railMat.roughness = 0.4 + wear * 0.3;
			rungMat.albedoColor = new Color3(0.32 * wearFactor, 0.32 * wearFactor, 0.35 * wearFactor);
			rungMat.metallic = 0.8;
			rungMat.roughness = 0.45 + wear * 0.3;
		} else if (material === "wood") {
			railMat.albedoColor = new Color3(0.45 * wearFactor, 0.32 * wearFactor, 0.18 * wearFactor);
			railMat.metallic = 0;
			railMat.roughness = 0.8;
			rungMat.albedoColor = new Color3(0.5 * wearFactor, 0.35 * wearFactor, 0.2 * wearFactor);
			rungMat.metallic = 0;
		...
		const ladderWidth = type === "cage" ? 0.5 : type === "stepladder" ? 0.6 : 0.4;
		...
		if (type === "fixed" || type === "extension") {
			...
			for (const side of [-1, 1]) {
				...
				rail.position = new Vector3(
					posX + Math.cos(rotation) * (side * ladderWidth / 2) + Math.sin(lean) * leanOffset / 2,
					posY + height / 2,
			...
			for (let b = 0; b < barCount; b++) {
				...
				bar.position = new Vector3(
					posX - Math.cos(rotation) * (cageRadius / 2) + Math.cos(barAngle + rotation) * cageRadius,
					posY + height / 2 + 0.25,
					posZ + Math.sin(rotation) * (cageRadius / 2) - Math.sin(barAngle + rotation) * cageRadius
				);
			...
			for (const side of [-1, 1]) {
				...
				leg.position = new Vector3(
					posX + Math.cos(rotation) * (side * ladderWidth / 2) + Math.sin(rotation) * Math.sin(spreadAngle) * height / 2,
					posY + Math.cos(spreadAngle) * height / 2,
					posZ - Math.sin(rotation) * (side * ladderWidth / 2) + Math.cos(rotation) * Math.sin(spreadAngle) * height / 2
				);
			...
			for (const side of [-1, 1]) {
				...
				leg.position = new Vector3(
					posX + Math.cos(rotation) * (side * ladderWidth / 2) - Math.sin(rotation) * Math.sin(spreadAngle) * height * 0.47,
					posY + Math.cos(spreadAngle) * height * 0.47,
					posZ - Math.sin(rotation) * (side * ladderWidth / 2) - Math.cos(rotation) * Math.sin(spreadAngle) * height * 0.47
				);
				leg.rotation.x = -spreadAngle;
				leg.rotation.y = rotation;
				leg.material = railMat;
	...
	}, [scene, id, posX, posY, posZ, type, material, height, rotation, lean, wear, seed]);
