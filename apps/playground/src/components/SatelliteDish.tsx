...
const DISH_SIZES: Record<SatelliteDishType, { diameter: number; focalLength: number; feedSize: number }> = {
...
const CONDITION_FACTORS: Record<ConditionType, { rust: number; roughness: number; albedo: number }> = {
...
export function SatelliteDish({
	...
	useEffect(() => {
		...
		const focalLength = sizeSpec.focalLength * (actualDiameter / sizeSpec.diameter);
		...
		const brokenElevation = isBroken && rng
			? elevationRad + (rng.next() - 0.5) * 0.5
			: elevationRad;
		const brokenAzimuth = isBroken && rng
			? azimuthRad + (rng.next() - 0.5) * 0.3
			: azimuthRad;
		...
		if (type === "commercial") {
			...
			pedestal.position = new Vector3(posX, posY + 0.04 + mountHeight * 0.35, posZ);
			...
			pivotHousing.position = new Vector3(posX, posY + 0.04 + mountHeight * 0.7 + 0.075, posZ);
			...
			collar.position = new Vector3(posX, posY + 0.08 + mountHeight + 0.03, posZ);
			...
			arm.position = new Vector3(
				posX + Math.sin(brokenAzimuth) * armLength * 0.4 * Math.cos(brokenElevation),
				posY + mountHeight + 0.06 + armLength * 0.4 * Math.sin(brokenElevation),
				posZ + Math.cos(brokenAzimuth) * armLength * 0.4 * Math.cos(brokenElevation)
			);
		...
		dish.position = new Vector3(
			posX + Math.sin(brokenAzimuth) * dishDistFromMount * Math.cos(brokenElevation),
			dishCenterY + dishDistFromMount * Math.sin(brokenElevation),
			posZ + Math.cos(brokenAzimuth) * dishDistFromMount * Math.cos(brokenElevation)
		);
		...
		const feedX = posX + Math.sin(brokenAzimuth) * (dishDistFromMount + feedDistance) * Math.cos(brokenElevation);
		const feedY = dishCenterY + (dishDistFromMount + feedDistance) * Math.sin(brokenElevation);
		const feedZ = posZ + Math.cos(brokenAzimuth) * (dishDistFromMount + feedDistance) * Math.cos(brokenElevation);
		...
		if (type === "commercial") {
			...
			for (let i = 0; i < 4; i++) {
				...
				const strutStart = new Vector3(
					...
					dish.position.y + Math.sin(i % 2 === 0 ? 0.1 : -0.1) * actualDiameter * 0.1,
					dish.position.z + Math.sin(strutAngle) * (actualDiameter * 0.4)
				);

				const strutLength = Vector3.Distance(strutStart, new Vector3(feedX, feedY, feedZ));
				...
				const direction = new Vector3(feedX - strutStart.x, feedY - strutStart.y, feedZ - strutStart.z).normalize();
			...
			for (let i = 0; i < armCount; i++) {
				...
				const armStart = new Vector3(
					dish.position.x + Math.cos(armAngle) * (actualDiameter * 0.35) * Math.cos(brokenElevation),
					dish.position.y,
					dish.position.z + Math.sin(armAngle) * (actualDiameter * 0.35) * Math.cos(brokenElevation)
				);

				const feedArmLength = Vector3.Distance(armStart, new Vector3(feedX, feedY, feedZ));
				...
				const armDir = new Vector3(feedX - armStart.x, feedY - armStart.y, feedZ - armStart.z).normalize();
		...
		lnbHousing.position = new Vector3(
			feedX + Math.sin(brokenAzimuth) * feedSize * 1.5 * Math.cos(brokenElevation),
			feedY + feedSize * 1.5 * Math.sin(brokenElevation),
			feedZ + Math.cos(brokenAzimuth) * feedSize * 1.5 * Math.cos(brokenElevation)
		);
		...
		if (condition === "pristine" || condition === "weathered") {
			...
			const label = MeshBuilder.CreateBox(
				`${id}_label`,
				{ width: actualDiameter * 0.15, height: actualDiameter * 0.05, depth: 0.003 },
				scene
			);
			label.position = new Vector3(
				dish.position.x - Math.sin(brokenAzimuth) * (actualDiameter * 0.3) * Math.cos(brokenElevation - 0.2),
				dish.position.y - (actualDiameter * 0.15),
				dish.position.z - Math.cos(brokenAzimuth) * (actualDiameter * 0.3) * Math.cos(brokenElevation - 0.2)
			);
		...
		if ((condition === "rusted" || condition === "damaged") && rng && rng.next() < 0.6) {
	...
	}, [scene, id, posX, posY, posZ, type, diameter, elevation, azimuth, isBroken, condition, rotation, seed]);
