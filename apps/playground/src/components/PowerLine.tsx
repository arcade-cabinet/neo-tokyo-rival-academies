...
export function PowerLine({
	...
	useEffect(() => {
		...
		const createPole = (poleX: number, poleZ: number, suffix: string) => {
			...
			if (type === "utility" || type === "telephone") {
				...
				for (let i = 0; i < insulatorCount; i++) {
					const insulatorX = (i - (insulatorCount - 1) / 2) * (crossarmWidth / insulatorCount);
				...
				if (type === "utility" && wireCount > 2) {
					...
					crossarm2.position = new Vector3(poleX, posY + poleHeight - 1.2, poleZ);
			...
			if (hasTransformer && suffix === "main") {
				const transformerMat = new PBRMaterial(`powerline_transformer_${id}`, scene);
				...
				transformer.position = new Vector3(poleX, posY + poleHeight * 0.6, poleZ + 0.2);
		...
		if (spanLength > 0) {
			...
			// Wires between poles
			const wireSpacing = type === "highVoltage" ? 1.5 : type === "tram" ? 0 : 0.4;
			const wireHeight = posY + poleHeight - (type === "highVoltage" ? 0.9 : 0.4);
			...
			for (let w = 0; w < actualWireCount; w++) {
				...
				for (let s = 0; s < segmentCount; s++) {
					...
					const x1 = posX + Math.cos(rotation) * (t1 * spanLength) + Math.sin(rotation) * wireOffset;
					const z1 = posZ - Math.sin(rotation) * (t1 * spanLength) + Math.cos(rotation) * wireOffset;
					const y1 = wireHeight - sag1;

					const x2 = posX + Math.cos(rotation) * (t2 * spanLength) + Math.sin(rotation) * wireOffset;
					const z2 = posZ - Math.sin(rotation) * (t2 * spanLength) + Math.cos(rotation) * wireOffset;
					const y2 = wireHeight - sag2;

					const segLength = Math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2 + (z2 - z1) ** 2);
					const midX = (x1 + x2) / 2;
					const midY = (y1 + y2) / 2;
					const midZ = (z1 + z2) / 2;
	...
	}, [scene, id, posX, posY, posZ, type, poleHeight, spanLength, hasTransformer, wireSag, wireCount, rotation, seed]);
