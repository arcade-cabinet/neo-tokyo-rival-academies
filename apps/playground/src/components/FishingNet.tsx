...
export function FishingNet({
	...
	useEffect(() => {
		...
		if (type === "cast" || type === "decorative") {
			...
			const netCover = MeshBuilder.CreateCylinder(
				`${id}_cover`,
				{ height: trapHeight * 0.95, diameter: trapRadius * 1.9, tessellation: 16 },
				scene
			);
		...
		if (hasFloats && type !== "trap") {
			...
			for (let f = 0; f < floatCount; f++) {
				const angle = type === "cast"
					? (f / floatCount) * Math.PI * 2 + rotation
					: 0;
				const floatX = type === "cast"
					? posX + Math.cos(angle) * size
					: posX - Math.sin(rotation) * ((f / (floatCount - 1) - 0.5) * size * 2);
				const floatZ = type === "cast"
					? posZ + Math.sin(angle) * size
					: posZ - Math.cos(rotation) * ((f / (floatCount - 1) - 0.5) * size * 2);
				...
				float.position = new Vector3(floatX, posY + (type === "gill" ? size / 2 : 0), floatZ);
		...
		if (hasWeights && type !== "trap") {
			...
			for (let w = 0; w < weightCount; w++) {
				const angle = type === "cast"
					? (w / weightCount) * Math.PI * 2 + rotation
					: 0;
				const weightX = type === "cast"
					? posX + Math.cos(angle) * size
					: posX - Math.sin(rotation) * ((w / (weightCount - 1) - 0.5) * size * 2);
				const weightZ = type === "cast"
					? posZ + Math.sin(angle) * size
					: posZ - Math.cos(rotation) * ((w / (weightCount - 1) - 0.5) * size * 2);
				...
				weight.position = new Vector3(
					weightX,
					posY - (type === "cast" && state === "drying" ? size * 0.5 : size / 2),
					weightZ
				);
	...
	}, [scene, id, posX, posY, posZ, type, state, size, hasFloats, hasWeights, condition, rotation, seed]);
