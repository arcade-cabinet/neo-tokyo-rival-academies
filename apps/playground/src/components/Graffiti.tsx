...
export type GraffitiType = "tag" | "throw_up" | "piece" | "stencil" | "mural" | "scratch";
...
export interface GraffitiProps {
	...
	size?: GraffitiSize;
	/** Primary color */
	primaryColor?: "red" | "blue" | "green" | "yellow" | "purple" | "orange" | "white" | "black";
	/** Has drips */
	hasDrips?: boolean;
	/** Age/faded 0-1 */
	age?: number;
...
export function Graffiti({
	...
	useEffect(() => {
		...
		const getColor = (colorName: string): Color3 => {
			...
			switch (colorName) {
				case "red": return new Color3(0.85, 0.15, 0.1).scale(ageFactor);
				case "blue": return new Color3(0.1, 0.3, 0.85).scale(ageFactor);
				case "green": return new Color3(0.1, 0.7, 0.2).scale(ageFactor);
				case "yellow": return new Color3(0.9, 0.85, 0.1).scale(ageFactor);
				case "purple": return new Color3(0.6, 0.1, 0.7).scale(ageFactor);
				case "orange": return new Color3(0.95, 0.5, 0.1).scale(ageFactor);
				case "white": return new Color3(0.95, 0.95, 0.95).scale(ageFactor);
				case "black": return new Color3(0.1, 0.1, 0.12);
				default: return new Color3(0.85, 0.15, 0.1).scale(ageFactor);
			}
		};
		...
		if (type === "tag") {
			...
			const secondaryColorName = colors[rng ? Math.floor(rng.next() * colors.length) : 0];
			...
			for (let sh = 0; sh < shapeCount; sh++) {
				...
				const shx = (rng ? (rng.next() - 0.5) : (sh / shapeCount - 0.5)) * width * 0.7;
			...
			for (let d = 0; d < detailCount; d++) {
				...
				const detailColorName = detailColors[rng ? Math.floor(rng.next() * detailColors.length) : d % detailColors.length];
		...
		if (hasDrips && type !== "scratch" && type !== "stencil") {
			...
			for (let dr = 0; dr < dripCount; dr++) {
				const dripX = (rng ? (rng.next() - 0.5) : (dr / dripCount - 0.5)) * width * 0.6;
	...
	}, [scene, id, posX, posY, posZ, type, size, primaryColor, hasDrips, age, rotation, seed]);
