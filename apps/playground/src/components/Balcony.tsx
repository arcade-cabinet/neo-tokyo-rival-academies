...
const MATERIAL_COLORS: Record<BalconyMaterial, { base: Color3; metallic: number; roughness: number }> = {
...
export function Balcony({
	...
	useEffect(() => {
		...
		if (type === "standard" || type === "cantilevered") {
			...
			floor.position = new Vector3(posX, posY - floorThickness / 2, posZ + depth / 2);
			...
			frontRail.position = new Vector3(posX, posY + railingHeight, posZ + depth);
			...
			infill.position = new Vector3(posX, posY + railingHeight * 0.45, posZ + depth);
			...
			topRail.position = new Vector3(posX, posY + railingHeight, posZ + railDepth / 2);
			...
			frontGlass.position = new Vector3(posX, posY + wallHeight / 2, posZ + depth);
		...
		if (decorated && rng && (type === "standard" || type === "enclosed")) {
			...
			for (let i = 0; i < plantCount; i++) {
				...
				plant.position = new Vector3(potX, posY + 0.35 + rng.next() * 0.1, potZ);
			...
			if (rng.next() > 0.5) {
				...
				pole.position = new Vector3(posX, posY + railingHeight * 0.8, posZ + depth * 0.7);
	...
	}, [scene, id, posX, posY, posZ, type, material, width, depth, railingHeight, decorated, rotation, seed]);
