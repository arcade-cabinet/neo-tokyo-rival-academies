...
export function Bonfire({
	...
	useEffect(() => {
		...
		if (type === "barrel") {
			...
			const bowl = MeshBuilder.CreateCylinder(
				`${id}-bowl`,
				{ diameterTop: 0.8 * size, diameterBottom: 0.5 * size, height: 0.4 * size, tessellation: 8 },
				scene
			);
		...
		for (let i = 0; i < logCount; i++) {
			...
			const baseHeight = type === "barrel" ? 0.9 * size : type === "brazier" ? 0.55 * size : 0.05 * size;
		...
		if (state === "burning" || state === "smoldering" || state === "starting") {
			const fireHeight = type === "barrel" ? 0.9 * size : type === "brazier" ? 0.6 * size : 0.15 * size;
		...
		if (state === "burning" || state === "smoldering" || state === "starting") {
			const fireHeight = type === "barrel" ? 1 * size : type === "brazier" ? 0.7 * size : 0.3 * size;
	...
	}, [id, posX, posY, posZ, type, state, size, hasCookingGrate, fuelAmount, lightIntensity, rotation, seed, scene]);
