...
export function regenerateStability(
  ...
  const timeSinceLastDamage =
    stability.lastDamageTime === 0 ? Infinity : currentTime - stability.lastDamageTime;
 ...
 */
export function isBroken(breakState: BreakState | undefined, currentTime: number): boolean {
...
export function processHitWithStability(
  ...
  const { stability, breakTriggered } = reduceStability(entity.stability, damage, currentTime);
...
export function updateStabilityAndBreak(
  ...
  if (updatedEntity.breakState) {
    const newBreakState = updateBreakState(updatedEntity.breakState, currentTime);
  ...
  if (updatedEntity.stability && !isBroken(updatedEntity.breakState, currentTime)) {
 ...
 */
export function getBreakProgress(breakState: BreakState | undefined, currentTime: number): number {
