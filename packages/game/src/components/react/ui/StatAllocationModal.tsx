			...
			<div ref={modalRef} style={styles.modal}>
				...
				<div style={{ marginBottom: "20px", textAlign: "center" }}>
					<p style={{ fontSize: "1.2rem" }}>
						Available Points:{" "}
						<span style={{ color: "#00ffff" }}>{remainingPoints}</span>
					</p>
				</div>
						...
						>
							<div style={{ flex: 1 }}>
								...
								<div style={{ fontSize: "0.9rem", color: "#888" }}>
									Current: {currentStats[stat]} → New:{" "}
									{currentStats[stat] + allocation[stat]}
								</div>
							</div>
				...
				<div style={{ marginBottom: "20px" }}>
					<p
						style={{ marginBottom: "10px", fontSize: "0.9rem", color: "#888" }}
					>
						Quick Allocations:
					</p>
					<div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
						{(["tank", "melee_dps", "ranged_dps", "balanced"] as const).map(
							(role) => (
								<button
									key={role}
									type="button"
