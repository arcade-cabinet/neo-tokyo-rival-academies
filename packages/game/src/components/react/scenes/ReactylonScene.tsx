...
const HexTileGroup: FC<{
	positions: Vector3[];
	texturePath: string;
	index: number;
}> = ({ positions, texturePath, index }) => {
...
const WallBackdrops: FC = () => {
	...
	useEffect(() => {
		...
		import("@babylonjs/core/Meshes/Builders/planeBuilder").then(
			({ CreatePlane }) => {
				import("@babylonjs/core/Materials/standardMaterial").then(
					({ StandardMaterial }) => {
						import("@babylonjs/core/Materials/Textures/texture").then(
							({ Texture }) => {
								if (cancelled) return;
...
const KaiCharacter: FC = () => {
	...
	}, [scene, maxX, maxZ, keysRef.current]);
...
const SceneLighting: FC = () => {
	...
	useEffect(() => {
		...
		Promise.all([
			...
			// Neon accent lights
			const neonMagenta = new PointLight(
				"neon-magenta",
				new Vector3(-6, 4, -6),
				scene,
			);
			...
			const neonOrange = new PointLight(
				"neon-orange",
				new Vector3(0, 2, -8),
				scene,
			);
