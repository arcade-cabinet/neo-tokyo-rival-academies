...
class AISystem {
	...
	syncEntity(
		ecsEntity: ECSEntity & { id: string },
		defaultFaction: "ENEMY" | "ALLY",
	) {
		...
		if (!agent) {
			...
			if (
				ecsEntity.isBoss ||
				(defaultFaction === "ENEMY" && ecsEntity.modelColor === 0xffffff)
			) {
				faction = "BOSS";
			}
	...
	writeBackVelocity(
		ecsEntities: Iterable<{ id?: string; velocity?: { x: number; y: number } }>,
	) {
