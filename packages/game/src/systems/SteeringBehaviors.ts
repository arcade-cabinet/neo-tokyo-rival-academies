 ...
 */
export function pursue(agent: SteeringAgent, target: Vector3, targetVelocity: Vector3): Vector3 {
 ...
 */
export function evade(agent: SteeringAgent, target: Vector3, targetVelocity: Vector3): Vector3 {
  const distance = Vector3.Distance(agent.position, target);
  const lookAhead = distance / agent.maxSpeed;
 ...
 */
export function arrive(agent: SteeringAgent, target: Vector3, slowingRadius: number): Vector3 {
  const desired = target.subtract(agent.position);
