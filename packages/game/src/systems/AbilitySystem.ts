...
const abilityDatabase: Record<string, Ability[]> = abilitiesData as Record<
	string,
	Ability[]
>;
...
export function executeAbility(
	...
	let effect:
		| { type: Ability["effectType"]; value: number; target: ECSEntity }
		| undefined;
	...
	switch (ability.effectType) {
		...
		case "heal":
			if (
				...
				target.stats?.structure !== undefined
			) {
				const maxHealth = target.stats.structure;
				target.health = Math.min(
					maxHealth,
					target.health + ability.effectValue,
				);
