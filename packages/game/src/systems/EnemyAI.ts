...
import { type AIContext, AIState, createAIStateMachine, type PatrolState } from './AIStateMachine';
 ...
 */
export function createGruntAI(entityId: string, position: Vector3): StateMachine<AIContext> {
 ...
 */
export function createBossAI(entityId: string, position: Vector3): StateMachine<AIContext> {
  const context: AIContext = {
    entityId,
    position,
    targetPosition: null,
