				...
				fc.property(
					...
					(availablePoints, allocation) => {
						...
						if (result.success) {
							...
							const pointsSpent =
								initialPoints - (entity.level?.statPoints ?? 0);
							expect(pointsSpent).toBe(totalAllocated);

							// Remaining points should be correct
							expect(entity.level?.statPoints).toBe(
				...
				fc.property(
					...
					(initialStats, allocation) => {
						...
						if (result.success && entity.stats) {
							...
							expect(entity.stats.structure).toBeGreaterThanOrEqual(
								initialStats.structure,
							);
							expect(entity.stats.ignition).toBeGreaterThanOrEqual(
								initialStats.ignition,
							);
							expect(entity.stats.logic).toBeGreaterThanOrEqual(
								initialStats.logic,
							);
							expect(entity.stats.flow).toBeGreaterThanOrEqual(
								initialStats.flow,
							);
						}
					},
