...
const dynamicEntities = ECS.world.with(
	"position",
	"velocity",
	"characterState",
);
...
export const PhysicsSystem = () => {
	useFrame((_state, delta) => {
		...
		for (const entity of dynamicEntities) {
			...
			if (
				entity.characterState === "stand" ||
				entity.characterState === "block"
			) {
				// Friction
				entity.velocity.x *= CONFIG.FRICTION_MULTIPLIER;
				if (Math.abs(entity.velocity.x) < CONFIG.VELOCITY_THRESHOLD)
					entity.velocity.x = 0;
			}
			...
			const isGrounded =
				...
				(entity.position.y <= groundHeight + 0.1 ||
					(entity.characterState !== "jump" &&
						entity.position.y <= groundHeight + snapDist));
