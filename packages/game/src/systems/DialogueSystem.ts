...
export const startDialogue = (entityId: string, dialogueId: string) => {
	...
	if (
		...
		console.warn(
			`Could not start dialogue: ${dialogueId} for entity ${entityId}`,
		);
	}
};
...
export const advanceDialogue = (entityId: string) => {
	...
	if (entity?.dialogueState?.isInteracting) {
		const dialogueSequence =
			storyData.dialogues[entity.dialogueState.currentDialogueId];
		if (!dialogueSequence) return;

		// Optional chaining fix
		const currentNode = dialogueSequence.find(
			(n) => n.id === entity.dialogueState?.nodeId,
		);
...
export const getCurrentDialogueNode = (entityId: string) => {
	...
	if (entity?.dialogueState?.isInteracting) {
		...
		// Optional chaining fix
		return (
			dialogueSequence.find((n) => n.id === entity.dialogueState?.nodeId) ||
			null
		);
	}
	return null;
