...
export function normalizeToHex(
  ...
  return {
    scene,
    scale: cfg.overflowMode === 'scale' ? scale : Math.min(scaleX, scaleY, scaleZ),
    originalBounds,
    normalizedBounds,
    wasClipped,
  };
 ...
 */
function applyHexClipping(scene: THREE.Group, config: HexNormalizerConfig): boolean {
  let didClip = false;
...
function clipGeometryToHex(
  ...
  for (let i = 0; i < positions.length; i += 3) {
    ...
    if (!isInsideHex(x, z)) {
      ...
      const [clippedX, clippedZ] = projectToHexBoundary(x, z, effectiveHexSize, config.orientation);
