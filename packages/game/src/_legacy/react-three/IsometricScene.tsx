...
function WallBackdrops() {
	...
	return (
		<group>
			...
			{/* Left wall */}
			<mesh
				position={[-wallOffset, wallHeight / 2 - 2, 0]}
...
function KaiCharacter() {
	...
	const { actions } = useAnimations(
		[...idleAnim.animations, ...runAnim.animations],
		group,
	);
	...
	useEffect(() => {
		...
		const idle =
			findActionByPattern(actions, "combat") ||
			findActionByPattern(actions, "stance");
	...
	useFrame(() => {
		...
		rigidBody.current.setLinvel(
			{ x: velX, y: rigidBody.current.linvel().y, z: velZ },
			true,
		);
