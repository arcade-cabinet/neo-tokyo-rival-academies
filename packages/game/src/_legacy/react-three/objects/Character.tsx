...
export function Character({
	...
	useFrame((frameState, delta) => {
		...
		if (state === "stand") {
			...
			// Reset rotations
			if (limbsRef.current.legL.current)
				limbsRef.current.legL.current.rotation.x = 0;
			if (limbsRef.current.legR.current)
				limbsRef.current.legR.current.rotation.x = 0;
			if (limbsRef.current.armL.current)
				limbsRef.current.armL.current.rotation.x = 0;
			if (limbsRef.current.armR.current)
				limbsRef.current.armR.current.rotation.x = 0;
			...
			if (limbsRef.current.legL.current)
				limbsRef.current.legL.current.rotation.x = 0.2;
			if (limbsRef.current.legR.current)
				limbsRef.current.legR.current.rotation.x = -0.2;
			...
			if (limbsRef.current.armL.current)
				limbsRef.current.armL.current.rotation.z = 0;
			if (limbsRef.current.armR.current)
				limbsRef.current.armR.current.rotation.z = 0;

			if (limbsRef.current.legL.current)
				limbsRef.current.legL.current.rotation.x = Math.sin(run);
			if (limbsRef.current.legR.current)
			...
			if (limbsRef.current.armR.current)
				limbsRef.current.armR.current.rotation.x = Math.sin(run);
		} else if (state === "jump") {
			pivotRef.current.rotation.x = 0;
			if (limbsRef.current.legL.current)
				limbsRef.current.legL.current.rotation.x = 0.5;
			if (limbsRef.current.legR.current)
				limbsRef.current.legR.current.rotation.x = -0.2;
			if (limbsRef.current.armL.current)
				limbsRef.current.armL.current.rotation.x = -2.5;
			if (limbsRef.current.armR.current)
				limbsRef.current.armR.current.rotation.x = -2;
		} else if (state === "slide") {
			...
			if (limbsRef.current.legL.current)
				limbsRef.current.legL.current.rotation.x = 1.5;
			if (limbsRef.current.legR.current)
				limbsRef.current.legR.current.rotation.x = 1.3;
			if (limbsRef.current.armL.current)
				limbsRef.current.armL.current.rotation.x = 0.5;
			if (limbsRef.current.armR.current)
				limbsRef.current.armR.current.rotation.x = 0.5;
		} else if (state === "stun") {
			pivotRef.current.rotation.x = -0.5;
			if (limbsRef.current.armL.current)
				limbsRef.current.armL.current.rotation.x = -1;
			if (coatSeg1Ref.current) coatSeg1Ref.current.rotation.x = -2;
		} else if (state === "attack") {
			pivotRef.current.rotation.x = 0.2;
			// Rapid strikes
			if (limbsRef.current.armR.current) {
				limbsRef.current.armR.current.rotation.x =
					-Math.PI / 2 + Math.sin(t * 30) * 1.0;
				limbsRef.current.armR.current.rotation.z = 0.5;
			}
			if (limbsRef.current.legL.current)
				limbsRef.current.legL.current.rotation.x = 0.5;
			if (limbsRef.current.legR.current)
				limbsRef.current.legR.current.rotation.x = -0.5;
		}
	});
