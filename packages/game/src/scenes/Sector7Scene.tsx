...
const CONFIG = STAGE_CONFIGS.sector7_streets;
...
const DATA_SHARDS: DataShardLocation[] = [
	{
		id: "s7_shard_1",
...
export function Sector7Scene({ onStageComplete, inputState }: Sector7SceneProps) {
	...
	useEffect(() => {
		...
		const streetGround = MeshBuilder.CreateGround(
			"street",
			{ width: 100, height: 25 },
			scene,
		);
	...
	return (
		<>
			...
			<IsometricCamera
				target={new Vector3(playerProgress, 0, 0)}
				radius={30}
				orthoSize={18}
			/>
