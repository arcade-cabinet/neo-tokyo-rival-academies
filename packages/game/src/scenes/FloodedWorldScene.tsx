...
function generateRooftopLayout(
	...
	for (let i = 1; i < count; i++) {
		...
		const heightDiff = Math.abs(source.height - height);
		const avgHeight = (source.height + height) / 2;
...
function RooftopRenderer({
	...
	const props = useMemo(() => {
		...
		for (let i = 0; i < propCount; i++) {
			...
			const margin = 1.5;
			const x =
				position.x + (propRng.next() - 0.5) * (width - margin * 2);
			const z =
				position.z + (propRng.next() - 0.5) * (depth - margin * 2);
...
export function FloodedWorldScene({
	seedPhrase,
	onExit,
}: FloodedWorldSceneProps) {
	...
	return (
		<>
			...
			<Character
				id="hero"
				position={spawnPoint}
				modelPath="/assets/characters/hero.glb"
			/>
