...
import { type StageConfig, type StageId } from "./SceneManager";
...
export function BaseGameScene({
	...
	useEffect(() => {
		...
		scene.clearColor = (
			clearColors[config.theme] || clearColors.dark
		).toColor4(1);
...
function createParallaxBackgrounds(
	...
	for (const layerConfig of layerConfigs) {
		...
		const assetPath = config.assets.backgrounds.find((bg) =>
			bg.includes(layerConfig.pattern),
		);
