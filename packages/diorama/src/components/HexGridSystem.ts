...
import type { GridBounds, GridConfig } from "../types/grid";
import { type HexTile, TileType } from "../types/tiles";
import { getHexesInBounds, hexToWorld } from "../utils/hex-grid";
...
export const HexGridSystem = {
	...
	generateGrid(config: GridConfig): HexTile[] {
		...
		const hexCoords = getHexesInBounds(
			bounds.minX,
			bounds.maxX,
			bounds.minZ,
			bounds.maxZ,
	...
	getEdgeTiles(
		...
		const leftEdge = tiles.filter(
			(tile) => Math.abs(tile.worldX - bounds.minX) < threshold,
		);
		const rightEdge = tiles.filter(
			(tile) => Math.abs(tile.worldX - bounds.maxX) < threshold,
		);
