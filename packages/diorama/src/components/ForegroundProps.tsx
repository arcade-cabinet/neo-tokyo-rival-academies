...
export function ForegroundProps({ seed, bounds, enableParticles = true }: ForegroundPropsProps) {
...
function createPipeCluster(
  ...
  for (let i = 0; i < pipeConfigs.length; i++) {
    ...
    pipe.position = new Vector3(position.x + rotatedX, cfg.height / 2, position.z + rotatedZ);
...
function createSteamParticles(scene: Scene, position: Vector3): ParticleSystem {
  const particleSystem = new ParticleSystem(`steam_${position.x}_${position.z}`, 200, scene);
