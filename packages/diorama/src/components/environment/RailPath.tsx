...
export function RailPath({
  ...
  useEffect(() => {
    ...
    switch (pathType) {
      ...
      case 'bezier':
        ...
        if (points.length === 3) {
          curve = Curve3.CreateQuadraticBezier(points[0], points[1], points[2], subdivisions);
    ...
    if (debug) {
      ...
      points.forEach((point, i) => {
        const marker = MeshBuilder.CreateSphere(`railMarker_${id}_${i}`, { diameter: 0.3 }, scene);
  ...
  }, [scene, id, points, pathType, subdivisions, debug, debugColor, onPathReady]);
...
export const RailPathPresets = {
   ...
   */
  lShape: (start: Vector3, corner: Vector3, end: Vector3): Vector3[] => [start, corner, end],
