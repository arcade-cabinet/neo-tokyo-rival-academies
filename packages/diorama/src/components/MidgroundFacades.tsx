...
export function MidgroundFacades({ minX, maxX, height = 25, depth = -8 }: MidgroundFacadesProps) {
  ...
  useEffect(() => {
    ...
    // Create materials
    const { buildingMat, windowMat, neonMat, roofMat } = createFacadeMaterials(scene);
...
function createWindowGrid(
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      windowMat.emissiveColor = new Color3(brightness * 0.6, brightness * 0.7, brightness);
...
function createFireEscape(
  ...
  for (let i = 0; i < platformCount; i++) {
    ...
    platform.position = new Vector3(position.x, position.y + 3 + i * 5, position.z);
 ...
 */
function createWaterTank(scene: Scene, name: string, position: Vector3): AbstractMesh {
 ...
 */
function createACUnit(scene: Scene, name: string, position: Vector3): AbstractMesh {
  const ac = MeshBuilder.CreateBox(name, { width: 2, height: 1.5, depth: 1.5 }, scene);
