...
export type WaterTankType = 'wooden' | 'metal' | 'concrete' | 'plastic' | 'industrial';
...
export function WaterTank({
  ...
  useEffect(() => {
    ...
    if (type === 'wooden') {
      tankMat.albedoColor = new Color3(0.4 * ageFactor, 0.32 * ageFactor, 0.2 * ageFactor);
      tankMat.metallic = 0;
      ...
      hatch.position = new Vector3(posX, tankY + height + diameter / 4 + 0.05, posZ);
      ...
      const cap = MeshBuilder.CreateCylinder(`${id}_cap`, { height: 0.08, diameter: 0.25 }, scene);
      cap.position = new Vector3(posX + diameter * 0.2, tankY + height + diameter * 0.35, posZ);
      cap.material = capMat;
      meshes.push(cap);
    } else if (type === 'industrial') {
      tankMat.albedoColor = new Color3(0.6 * ageFactor, 0.62 * ageFactor, 0.65 * ageFactor);
      ...
      for (const side of [-1, 1]) {
        ...
        cap.position = new Vector3(posX + side * diameter, tankY + diameter / 2, posZ);
      ...
      for (const side of [-1, 1]) {
        ...
        saddle.position = new Vector3(posX + side * diameter * 0.6, tankY + diameter / 4, posZ);
    ...
    inletPipe.position = new Vector3(posX - diameter / 2 + 0.1, tankY + height * 0.8, posZ);
    ...
    overflow.position = new Vector3(posX, tankY + height * 0.95, posZ + diameter / 2 - 0.1);
  ...
  }, [scene, id, posX, posY, posZ, type, diameter, height, hasSupport, supportHeight, age, seed]);
