...
export function Antenna({
  ...
  useEffect(() => {
    ...
    metalMat.albedoColor = new Color3(0.55 * rustFactor, 0.55 * rustFactor, 0.58 * rustFactor);
    metalMat.metallic = 0.85;
    metalMat.roughness = 0.35 + rust * 0.3;

    const poleMat = new PBRMaterial(`antenna_pole_${id}`, scene);
    poleMat.albedoColor = new Color3(0.5 * rustFactor, 0.5 * rustFactor, 0.52 * rustFactor);
    ...
    if (type === 'tv') {
      ...
      const lnb = MeshBuilder.CreateCylinder(`${id}_lnb`, { height: 0.08, diameter: 0.04 }, scene);
      ...
      // Base
      const base = MeshBuilder.CreateCylinder(`${id}_base`, { height: 0.2, diameter: 0.15 }, scene);
      base.position = new Vector3(posX, posY + 0.1, posZ);
      base.material = poleMat;
      meshes.push(base);

      // Coil section
      const coil = MeshBuilder.CreateCylinder(`${id}_coil`, { height: 0.3, diameter: 0.06 }, scene);
      ...
      for (let a = 0; a < antennaCount; a++) {
        const antennaX = ((a - (antennaCount - 1) / 2) * spread) / (antennaCount - 1 || 1);
        ...
        // Articulation joint
        const joint = MeshBuilder.CreateSphere(`${id}_joint_${a}`, { diameter: 0.025 }, scene);
  ...
  }, [scene, id, posX, posY, posZ, type, height, dishSize, rotation, rust, seed]);
