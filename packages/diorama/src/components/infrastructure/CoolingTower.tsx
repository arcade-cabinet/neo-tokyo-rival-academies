...
export function CoolingTower({
  ...
  useEffect(() => {
    ...
    if (type === 'evaporative') {
      ...
      for (let s = 0; s < shellSegments; s++) {
        ...
        if (nextT < throatPosition) {
          ...
          nextRadius = throatRadius + (topRadius - throatRadius) * localT ** 0.8;
        }
      ...
      ladderRail.position = new Vector3(posX + baseRadius + 0.15, posY + ladderHeight / 2, posZ);
    ...
    if (isRunning && hasFan) {
      ...
      const steam = MeshBuilder.CreateSphere(`${id}_steam`, { diameter: diameter * 0.5 }, scene);
