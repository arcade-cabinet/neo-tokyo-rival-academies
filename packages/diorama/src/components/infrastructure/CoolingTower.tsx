	...
	useEffect(() => {
		...
		if (type === "evaporative") {
			...
			if (hasFan) {
				...
				fanStack.position = new Vector3(
					posX,
					posY + height + height * 0.125,
					posZ,
				);
			...
			for (let c = 0; c < coilCount; c++) {
				...
				const coil = MeshBuilder.CreateBox(
					`${id}_coil_${c}`,
					{
						width: coolerWidth / 3.5,
						height: coolerHeight * 0.85,
						depth: coolerDepth * 0.8,
					},
					scene,
				);
				...
				for (let f = 0; f < finCount; f++) {
					const fin = MeshBuilder.CreateBox(
						`${id}_fin_${c}_${f}`,
						{
							width: coolerWidth / 3.5 - 0.02,
							height: 0.01,
							depth: coolerDepth * 0.75,
						},
						scene,
					);
			...
			for (const lx of [-1, 1]) {
				for (const lz of [-1, 1]) {
					...
					leg.position = new Vector3(
						posX +
							Math.cos(rotation) * ((lx * coolerWidth) / 2.5) -
							Math.sin(rotation) * ((lz * coolerDepth) / 3),
						posY + 0.15,
						posZ +
							Math.sin(rotation) * ((lx * coolerWidth) / 2.5) +
							Math.cos(rotation) * ((lz * coolerDepth) / 3),
					);
			...
			drySection.position = new Vector3(
				posX,
				posY + wetHeight + dryHeight / 2,
				posZ,
			);
			...
			for (let s = 0; s < shellSegments; s++) {
				...
				if (t < throatPosition) {
					...
					radius = baseRadius - (baseRadius - throatRadius) * localT ** 0.5;
				} else {
					const localT = (t - throatPosition) / (1 - throatPosition);
					radius = throatRadius + (topRadius - throatRadius) * localT ** 0.8;
				}
				...
				if (nextT < throatPosition) {
					...
					nextRadius = baseRadius - (baseRadius - throatRadius) * localT ** 0.5;
				} else {
					const localT = (nextT - throatPosition) / (1 - throatPosition);
					nextRadius =
						throatRadius + (topRadius - throatRadius) * localT ** 0.8;
				}
				...
				segment.position = new Vector3(
					posX,
					posY + s * segmentHeight + segmentHeight / 2,
					posZ,
				);
		...
		if (isRunning && hasFan) {
			...
			steam.position = new Vector3(
				posX,
				posY + height + (type === "industrial" ? 0.5 : height * 0.3),
				posZ,
			);
