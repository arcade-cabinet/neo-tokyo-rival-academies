    ...
    if (spanLength > 0) {
      ...
      for (let w = 0; w < actualWireCount; w++) {
        ...
        for (let s = 0; s < segmentCount; s++) {
          ...
          const x1 =
            posX + Math.cos(rotation) * (t1 * spanLength) + Math.sin(rotation) * wireOffset;
          const z1 =
            posZ - Math.sin(rotation) * (t1 * spanLength) + Math.cos(rotation) * wireOffset;
          const y1 = wireHeight - sag1;

          const x2 =
            posX + Math.cos(rotation) * (t2 * spanLength) + Math.sin(rotation) * wireOffset;
          const z2 =
            posZ - Math.sin(rotation) * (t2 * spanLength) + Math.cos(rotation) * wireOffset;
