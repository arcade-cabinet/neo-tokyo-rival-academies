...
export function Rope({
	...
	useEffect(() => {
		...
		switch (type) {
			case "hemp":
				ropeMat.albedoColor = new Color3(0.55, 0.45, 0.3).scale(
					conditionFactor,
				);
				ropeMat.metallic = 0;
				...
				break;
			case "nylon":
				ropeMat.albedoColor = new Color3(0.9, 0.85, 0.75).scale(
					conditionFactor,
				);
			...
			case "cable":
				...
				break;
			case "chain":
				ropeMat.albedoColor = new Color3(0.5, 0.52, 0.55).scale(
					conditionFactor,
				);
				...
				break;
			case "wire":
				ropeMat.albedoColor = new Color3(0.6, 0.62, 0.65).scale(
					conditionFactor,
				);
		...
		if (style === "straight") {
			...
			for (let i = 0; i <= segmentCount; i++) {
				...
				const x =
					(1 - t) * (1 - t) * posX + 2 * (1 - t) * t * midX + t * t * endPos.x;
				const y =
					(1 - t) * (1 - t) * posY + 2 * (1 - t) * t * midY + t * t * endPos.y;
				const z =
					(1 - t) * (1 - t) * posZ + 2 * (1 - t) * t * midZ + t * t * endPos.z;
