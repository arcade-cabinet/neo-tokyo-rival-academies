...
export type AirConditionerType = 'window' | 'split' | 'mini' | 'industrial' | 'portable';
...
export function AirConditioner({
  ...
  useEffect(() => {
    ...
    if (type === 'window') {
      ...
      controls.position = new Vector3(
        posX - Math.sin(rotation) * (depth / 2 + 0.01) + Math.cos(rotation) * (width * 0.3),
        posY + height * 0.3,
        posZ - Math.cos(rotation) * (depth / 2 + 0.01) - Math.sin(rotation) * (width * 0.3)
      );
      ...
      grilleMat.albedoColor = new Color3(0.4, 0.42, 0.45).scale(conditionFactor);
      ...
      if (isRunning) {
        ...
        for (let b = 0; b < bladeCount; b++) {
          ...
          blade.position = new Vector3(
            ...
            posZ - Math.cos(rotation) * (depth / 2 + 0.02) + Math.cos(bladeAngle) * height * 0.2
          );
      ...
      if (isRunning) {
        ...
        const led = MeshBuilder.CreateSphere(`${id}_led`, { diameter: 0.02, segments: 8 }, scene);
        led.position = new Vector3(
          posX - Math.sin(rotation) * (depth / 2 + 0.01) + Math.cos(rotation) * (width / 2 - 0.05),
          posY + height * 0.3,
          posZ - Math.cos(rotation) * (depth / 2 + 0.01) - Math.sin(rotation) * (width / 2 - 0.05)
        );
        led.material = ledMat;
        meshes.push(led);
      }
      ...
      const hose = MeshBuilder.CreateCylinder(`${id}_hose`, { height: 0.8, diameter: 0.12 }, scene);
  ...
  }, [scene, id, posX, posY, posZ, type, isRunning, hasDripStain, condition, rotation, seed]);
