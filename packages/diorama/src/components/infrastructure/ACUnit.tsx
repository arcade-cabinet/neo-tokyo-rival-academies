...
export function ACUnit({
  ...
  useEffect(() => {
    ...
    bodyMat.albedoColor = new Color3(0.85 - ageVariation, 0.85 - ageVariation, 0.87 - ageVariation);
    ...
    if (size === 'industrial' || size === 'large') {
      ...
      for (let i = 0; i < bladeCount; i++) {
        ...
        blade.position = new Vector3(
          posX +
            Math.sin(rotation) * (dims.depth / 2 - 0.03) +
            Math.cos(rotation + bladeAngle) * fanRadius * 0.45,
          posY + dims.height * 0.55 + Math.sin(bladeAngle) * fanRadius * 0.45,
          posZ +
            Math.cos(rotation) * (dims.depth / 2 - 0.03) -
            Math.sin(rotation + bladeAngle) * fanRadius * 0.45
        );
    ...
    for (let i = 0; i < ventCount; i++) {
      const vent = MeshBuilder.CreateBox(
        `${id}_vent_${i}`,
        {
          width: dims.width / (ventCount + 1),
          height: 0.02,
          depth: dims.depth * 0.6,
        },
        scene
      );
    ...
    for (let i = 0; i < 2; i++) {
      ...
      pipe.position = new Vector3(
        posX -
          Math.sin(rotation) * (dims.depth / 2 - 0.05) +
          Math.cos(rotation) * (dims.width / 2 - 0.1 + i * pipeSpacing),
        posY + dims.height + (wallMounted ? 0.25 : 0.15),
        posZ -
          Math.cos(rotation) * (dims.depth / 2 - 0.05) -
          Math.sin(rotation) * (dims.width / 2 - 0.1 + i * pipeSpacing)
      );
      ...
      for (const cx of [-1, 1]) {
        for (const cz of [-1, 1]) {
          ...
          foot.position = new Vector3(
            posX +
              Math.cos(rotation) * (cx * (dims.width / 2 - 0.08)) +
              Math.sin(rotation) * (cz * (dims.depth / 2 - 0.05)),
            posY + 0.025,
            posZ -
              Math.sin(rotation) * (cx * (dims.width / 2 - 0.08)) +
              Math.cos(rotation) * (cz * (dims.depth / 2 - 0.05))
          );
    ...
    if (age > 0.4 && rng) {
      ...
      for (let i = 0; i < streakCount; i++) {
        ...
        streak.position = new Vector3(
          posX +
            Math.sin(rotation) * (dims.depth / 2 + 0.02) +
            Math.cos(rotation) * ((rng.next() - 0.5) * dims.width * 0.7),
          posY + dims.height - streak.scaling.y * dims.height * 0.3,
          posZ +
            Math.cos(rotation) * (dims.depth / 2 + 0.02) -
            Math.sin(rotation) * ((rng.next() - 0.5) * dims.width * 0.7)
        );
