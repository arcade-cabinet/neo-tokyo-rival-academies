...
export function HeliPad({
	...
	useEffect(() => {
		...
		switch (type) {
			case "hospital":
				markingMat.albedoColor = new Color3(0.85, 0.15, 0.1).scale(
					conditionFactor,
				);
				break;
			case "corporate":
				markingMat.albedoColor = new Color3(0.95, 0.95, 0.95).scale(
					conditionFactor,
				);
				break;
			case "military":
				markingMat.albedoColor = new Color3(0.3, 0.35, 0.25).scale(
					conditionFactor,
				);
				break;
			case "emergency":
				markingMat.albedoColor = new Color3(0.95, 0.6, 0.1).scale(
					conditionFactor,
				);
				break;
		}
		...
		if (hasWindsock) {
			...
			sock.position = new Vector3(
				posX +
					Math.cos(rotation + Math.PI / 4) * (diameter / 2 + 1) +
					sockLength / 2,
				posY + poleHeight - 0.1,
				posZ - Math.sin(rotation + Math.PI / 4) * (diameter / 2 + 1),
			);
		...
		perimeterMat.albedoColor = new Color3(0.9, 0.85, 0.1).scale(
			conditionFactor,
		);
		...
		for (let seg = 0; seg < segmentCount; seg++) {
			if (seg % 2 === 0) {
				...
				segment.position = new Vector3(
					posX +
						Math.cos(segAngle + (nextAngle - segAngle) / 2) *
							(diameter / 2 + 0.3),
					posY + 0.16,
					posZ +
						Math.sin(segAngle + (nextAngle - segAngle) / 2) *
							(diameter / 2 + 0.3),
				);
