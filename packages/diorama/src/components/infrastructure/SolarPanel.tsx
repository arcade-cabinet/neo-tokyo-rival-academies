	...
	useEffect(() => {
		...
		if (type === "residential") {
				...
				for (let col = 0; col < cols && panelIndex < panelCount; col++) {
					...
					const cell = MeshBuilder.CreateBox(
						`${id}_cell_${panelIndex}`,
						{
							...
							height: panelThickness - 0.01,
							depth: panelDepth - 0.06,
						},
						scene,
					);
					...
					for (let g = 1; g < gridLines; g++) {
						...
						gridLine.position = new Vector3(
							posX +
								Math.cos(rotation) * offsetX -
								Math.sin(rotation) *
									(offsetZ + lineOffset * Math.cos(tiltAngle)),
							cellY + 0.022 + lineOffset * Math.sin(tiltAngle),
							posZ +
								Math.sin(rotation) * offsetX +
								Math.cos(rotation) *
									(offsetZ + lineOffset * Math.cos(tiltAngle)),
						);
			...
			for (const bx of [-1, 1]) {
				for (const bz of [-1, 1]) {
					const bracket = MeshBuilder.CreateBox(
						...
						{
							width: 0.05,
							height: bracketHeight + (Math.sin(tiltAngle) * panelDepth) / 2,
							depth: 0.05,
						},
						scene,
					);
					bracket.position = new Vector3(
						posX +
							Math.cos(rotation) * ((bx * totalWidth) / 3) -
							Math.sin(rotation) * ((bz * totalDepth) / 3),
						posY + bracketHeight / 2,
						posZ +
							Math.sin(rotation) * ((bx * totalWidth) / 3) +
							Math.cos(rotation) * ((bz * totalDepth) / 3),
					);
				...
				for (let col = 0; col < cols && row * cols + col < panelCount; col++) {
					...
					for (const side of ["top", "bottom", "left", "right"] as const) {
						...
						framePiece.position = new Vector3(
							posX +
								Math.cos(rotation) * (offsetX + frameOffsetX) -
								Math.sin(rotation) *
									(offsetZ + frameOffsetZ * Math.cos(tiltAngle)),
							panelY + frameOffsetZ * Math.sin(tiltAngle),
							posZ +
								Math.sin(rotation) * (offsetX + frameOffsetX) +
								Math.cos(rotation) *
									(offsetZ + frameOffsetZ * Math.cos(tiltAngle)),
						);
			...
			for (let rail = 0; rail < railCount; rail++) {
				const railZ =
					(rail / (railCount - 1) - 0.5) * (rows * (panelDepth + spacing));
				...
				for (const pz of [-1, 1]) {
					...
					post.position = new Vector3(
						posX +
							...
							Math.sin(rotation) * (pz * (panelDepth + spacing / 2)),
						posY + 0.2,
						posZ +
							Math.sin(rotation) * postX +
							Math.cos(rotation) * (pz * (panelDepth + spacing / 2)),
					);
			...
			for (let tp = 0; tp < trackerPanels; tp++) {
				...
				for (const clampZ of [-1, 1]) {
					...
					clamp.position = new Vector3(
						posX +
							Math.cos(rotation) * tpOffset -
							Math.sin(rotation) * ((clampZ * trackerDepth) / 3),
						posY + 0.8 - 0.03,
						posZ -
							Math.sin(rotation) * tpOffset +
							Math.cos(rotation) * ((clampZ * trackerDepth) / 3),
					);
			...
			for (let row = 0; row < rows && cellIndex < intPanels; row++) {
				for (let col = 0; col < cols && cellIndex < intPanels; col++) {
					...
					cell.position = new Vector3(
						posX +
							Math.cos(rotation) * cellOffsetX -
							Math.sin(rotation) * adjustedZ,
						adjustedY + 0.0075,
						posZ +
							Math.sin(rotation) * cellOffsetX +
							Math.cos(rotation) * adjustedZ,
					);
					cell.rotation.y = rotation;
					cell.rotation.x = -tiltAngle;
					cell.material = cellMat;
			...
			junctionBox.position = new Vector3(
				posX +
					Math.cos(rotation) * (intWidth / 2 - 0.1) -
					Math.sin(rotation) * (-intDepth / 2 + 0.1),
				posY + 0.05,
				posZ +
					Math.sin(rotation) * (intWidth / 2 - 0.1) +
					Math.cos(rotation) * (-intDepth / 2 + 0.1),
			);
			...
			conduit.position = new Vector3(
				posX +
					Math.cos(rotation) * (intWidth / 2 + 0.05) -
					Math.sin(rotation) * (-intDepth / 2 + 0.1),
				posY - 0.1,
				posZ +
					Math.sin(rotation) * (intWidth / 2 + 0.05) +
					Math.cos(rotation) * (-intDepth / 2 + 0.1),
			);
