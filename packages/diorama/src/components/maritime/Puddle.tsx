	...
	useEffect(() => {
		...
		if (condition === "stagnant") {
			...
			if (type === "rain" || type === "reflection") {
				puddleMat.albedoColor = puddleMat.albedoColor.add(
					new Color3(0.05, 0.08, 0.02),
				);
			}
		...
		for (let r = 0; r < rimSegments; r++) {
			const rimAngle =
				(r / rimSegments) * Math.PI * 2 + (rng ? rng.next() * 0.2 : 0);
		...
		if (hasRipples && condition !== "frozen") {
			...
			for (let rp = 0; rp < rippleCount; rp++) {
				...
				for (let ring = 0; ring < ringCount; ring++) {
					...
					rippleRing.position = new Vector3(
						posX + rippleX,
						posY + depth + 0.001,
						posZ + rippleZ,
					);
		...
		if (type === "oil") {
			...
			for (let p = 0; p < patchCount; p++) {
				...
				const patchAngle = rng
					? rng.next() * Math.PI * 2
					: (p / patchCount) * Math.PI * 2;
		...
		if (type === "mud" && condition !== "frozen") {
			...
			for (let d = 0; d < debrisCount; d++) {
				const debrisAngle = rng
					? rng.next() * Math.PI * 2
					: (d / debrisCount) * Math.PI * 2;
		...
		if (condition === "frozen") {
			...
			for (let c = 0; c < crackCount; c++) {
				...
				const crackAngle = rng
					? rng.next() * Math.PI * 2
					: (c / crackCount) * Math.PI * 2;
