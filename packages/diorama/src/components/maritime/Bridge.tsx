...
export function Bridge({
	...
	useEffect(() => {
		...
		if (type === "pedestrian") {
			...
			structMat.albedoColor = new Color3(0.4, 0.42, 0.45).scale(
				conditionFactor,
			);
			...
			structMat.albedoColor = new Color3(0.55, 0.53, 0.5).scale(
				conditionFactor,
			);
			...
			deckMat.metallic = 0;
			deckMat.roughness = 0.85;
			structMat.albedoColor = new Color3(0.55, 0.35, 0.25).scale(
				conditionFactor,
			);
			structMat.metallic = 0;
			...
			deckMat.metallic = 0.7;
			deckMat.roughness = 0.5;
			structMat.albedoColor = new Color3(0.5, 0.52, 0.55).scale(
				conditionFactor,
			);
			structMat.metallic = 0.8;
			structMat.roughness = 0.4;
			...
			for (let p = 0; p < pontoonCount; p++) {
				const pontoonX =
					(p / pontoonCount - 0.5) * length + length / pontoonCount / 2;
		...
		if (hasLights) {
			...
			for (let l = 0; l < lightCount; l++) {
				const lightX =
					(l / lightCount - 0.5) * length + length / lightCount / 2;
				for (const side of [-1, 1]) {
					// Light pole
