  ...
  useEffect(() => {
    ...
    const getCrateMaterial = (index: number): PBRMaterial => {
      ...
      if (type === 'wooden') {
        ...
        mat.albedoColor = new Color3(0.45 * shade, 0.35 * shade, 0.2 * shade).scale(
          conditionFactor
        );
        mat.metallic = 0;
        mat.roughness = 0.85;
        ...
        mat.albedoColor =
          colors[rng ? Math.floor(rng.next() * colors.length) : index % colors.length].scale(
            conditionFactor
          );
        mat.metallic = 0.1;
        mat.roughness = 0.6;
        ...
        mat.albedoColor =
          containerColors[
            rng ? Math.floor(rng.next() * containerColors.length) : index % containerColors.length
    ...
    for (let i = 0; i < cratePositions.length; i++) {
      ...
      crate.position = new Vector3(posX + rotatedX, posY + pos.y + pos.h / 2, posZ + rotatedZ);
      ...
      if (type === 'wooden' && rng && rng.next() > 0.5) {
        ...
        strap.position = new Vector3(posX + rotatedX, posY + pos.y + pos.h / 2, posZ + rotatedZ);
