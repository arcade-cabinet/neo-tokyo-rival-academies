...
export type CrateStackType =
	| "wooden"
	| "plastic"
	| "metal"
	| "mixed"
	| "shipping";
export type CrateStackArrangement =
	| "pyramid"
	| "tower"
	| "wall"
	| "scattered"
	...
	useEffect(() => {
		...
		const getCrateMaterial = (index: number): PBRMaterial => {
			...
			if (type === "wooden") {
				...
				mat.albedoColor =
					colors[
						rng ? Math.floor(rng.next() * colors.length) : index % colors.length
					].scale(conditionFactor);
				mat.metallic = 0.1;
				mat.roughness = 0.6;
			} else if (type === "metal") {
				...
				mat.albedoColor =
					containerColors[
						rng
							? Math.floor(rng.next() * containerColors.length)
		...
		if (arrangement === "pyramid") {
			...
			while (remaining > 0) {
				...
				for (let i = 0; i < cratesInLayer && remaining > 0; i++) {
					const lx =
						((i % actualLayerSize) - (actualLayerSize - 1) / 2) * crateSize;
					const lz =
						(Math.floor(i / actualLayerSize) - (actualLayerSize - 1) / 2) *
						crateSize;
				...
				for (let r = 0; r < ribCount; r++) {
					...
					for (const side of [-1, 1]) {
						...
						rib.position = new Vector3(
							posX +
								rotatedX +
								Math.cos(rotation + pos.rot) * ((side * pos.w) / 2),
							posY + pos.y + pos.h / 2,
