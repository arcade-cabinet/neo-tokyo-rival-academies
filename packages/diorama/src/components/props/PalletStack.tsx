...
export function PalletStack({
	...
	useEffect(() => {
		...
		if (type === "wooden" || type === "euro") {
			...
			palletMat.albedoColor = new Color3(0.15, 0.35, 0.55).scale(
				conditionFactor,
			);
			palletMat.metallic = 0.1;
			palletMat.roughness = 0.6;
		} else {
			palletMat.albedoColor = new Color3(0.5, 0.52, 0.55).scale(
				conditionFactor,
			);
		...
		const actualStackCount =
			state === "stacked" ? Math.max(stackCount, 2) : stackCount;
					...
					if (!isMissing) {
						...
						board.position = new Vector3(
							posX +
								stackOffset +
								Math.cos(rotation) * 0 -
								Math.sin(rotation) * boardZ,
							stackY + palletHeight - 0.01,
							posZ +
								stackOffset -
								Math.sin(rotation) * 0 -
								Math.cos(rotation) * boardZ,
						);
				...
				for (let fx = -1; fx <= 1; fx++) {
					for (let fz = -1; fz <= 1; fz++) {
						...
						foot.position = new Vector3(
							posX +
								stackOffset +
								Math.cos(rotation) * footX -
								Math.sin(rotation) * footZ,
							stackY + (palletHeight - 0.03) / 2,
							posZ +
								stackOffset -
								Math.sin(rotation) * footX -
								Math.cos(rotation) * footZ,
						);
		...
		if (hasCargo || state === "loaded") {
			...
			const wrap = MeshBuilder.CreateBox(
				`${id}_wrap`,
				{
					width: cargoWidth + 0.02,
					height: cargoHeight + 0.02,
					depth: cargoDepth + 0.02,
				},
				scene,
			);
