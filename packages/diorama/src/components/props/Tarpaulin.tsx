...
export function Tarpaulin({
	...
	useEffect(() => {
		...
		switch (color) {
			case "blue":
				tarpMat.albedoColor = new Color3(0.15, 0.35, 0.6).scale(
					conditionFactor,
				);
				break;
			case "green":
				...
				break;
			case "orange":
				tarpMat.albedoColor = new Color3(0.85, 0.45, 0.1).scale(
					conditionFactor,
				);
				break;
			case "gray":
				tarpMat.albedoColor = new Color3(0.5, 0.52, 0.55).scale(
					conditionFactor,
				);
				break;
			case "camo":
		...
		if (type === "flat") {
			...
			for (let w = 0; w < wrinkleCount; w++) {
				const wx =
					(rng ? rng.next() - 0.5 : w / wrinkleCount - 0.5) * width * 0.8;
			...
			const leftPanel = MeshBuilder.CreatePlane(
				`${id}_left`,
				{
					width: width / 2 / Math.cos(Math.atan2(tentHeight, width / 2)),
					height: ridgeLen,
				},
				scene,
			);
