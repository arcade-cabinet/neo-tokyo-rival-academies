...
const STATE_COLORS: Record<
	BonfireState,
	{ emissive: Color3; intensity: number }
> = {
	...
	useEffect(() => {
		...
		for (let i = 0; i < logCount; i++) {
			...
			if (state !== "dead" && random() > 0.5) {
				...
				charredMat.emissiveColor = stateConfig.emissive.scale(
					stateConfig.intensity * 0.3,
				);
		...
		if (
			hasCookingGrate &&
			(type === "campfire" || type === "pit" || type === "cooking")
		) {
		...
		if (rotation !== 0) {
			for (const mesh of meshes) {
				...
				mesh.position.x =
					posX + relX * Math.cos(rotation) - relZ * Math.sin(rotation);
				mesh.position.z =
					posZ + relX * Math.sin(rotation) + relZ * Math.cos(rotation);
