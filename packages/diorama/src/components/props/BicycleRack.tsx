...
const RACK_SPECS: Record<
	BicycleRackType,
	{ slotWidth: number; height: number; depth: number }
> = {
	...
	useEffect(() => {
		...
		if (type === "wave") {
			...
			for (let i = 0; i <= segments; i++) {
				...
				const y =
					height * (1 - Math.abs(Math.sin(t * waveCount * Math.PI)) * 0.3);
			...
			for (let i = 0; i < actualCapacity; i++) {
				...
				for (const side of [-1, 1]) {
					...
					anchor.position = transformPos(
						postX + (side * postWidth) / 2,
						0.005,
						0,
					);
					anchor.material = anchorMat;
			...
			for (const side of [-1, 1]) {
				...
				frameBar.position = transformPos(
					0,
					slotHeight,
					side * (slotDepth / 2 - 0.1),
				);
			...
			for (let i = 0; i < slotCount; i++) {
				...
				for (const side of [-1, 1]) {
					...
					guide.position = transformPos(slotX + side * 0.08, 0.15, 0);
			...
			for (let i = 0; i < actualCapacity; i++) {
				...
				hookArm.position = transformPos(
					hookX,
					height - 0.04,
					-depth / 2 + hookDepth / 2 + 0.02,
				);
				...
				for (let j = 0; j <= 8; j++) {
					...
					hookEndPath.push(
						new Vector3(0, Math.sin(angle) * 0.05, Math.cos(angle) * 0.05),
					);
				}
				...
				hookEnd.position = transformPos(
					hookX,
					height - 0.04,
					-depth / 2 + hookDepth + 0.02,
				);
				...
				lowerHook.position = transformPos(
					hookX,
					height * 0.4,
					-depth / 2 + hookDepth * 0.3 + 0.02,
				);
				...
				tray.position = transformPos(
					hookX,
					height * 0.38,
					-depth / 2 + hookDepth * 0.25 + 0.02,
				);
			...
			for (let b = 0; b < boltCount; b++) {
				const boltX =
					(b - (boltCount - 1) / 2) * (totalWidth / (boltCount - 1 || 1));
		...
		if (actualOccupied > 0) {
			...
			for (const slotIdx of occupiedSlots) {
				...
				const thisBikeMat = new PBRMaterial(
					`rack_bike_${id}_${slotIdx}`,
					scene,
				);
				...
				if (type === "wall") {
					...
					frame.position = transformPos(
						bikeX + (rng ? (rng.next() - 0.5) * 0.05 : 0),
						bikeY + 0.15,
						bikeZ,
					);
					...
					handlebar.position = transformPos(
						bikeX,
						bikeY + 0.45,
						bikeZ + wheelbase / 2 - 0.1,
					);
