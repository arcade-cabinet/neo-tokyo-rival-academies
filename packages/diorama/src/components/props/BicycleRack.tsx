    ...
    if (type === 'wave') {
      ...
      for (let i = 0; i < actualCapacity; i++) {
        ...
        for (let j = 0; j <= spiralSegments; j++) {
          ...
          spiralPath.push(
            new Vector3(Math.cos(angle) * spiralRadius, t * height, Math.sin(angle) * spiralRadius)
          );
        }
      ...
      for (let i = 0; i < actualCapacity; i++) {
        ...
        for (let j = 0; j <= 8; j++) {
          ...
          hookEndPath.push(new Vector3(0, Math.sin(angle) * 0.05, Math.cos(angle) * 0.05));
        }
      ...
      for (const slotIdx of occupiedSlots) {
        ...
        if (type === 'wall') {
          ...
          for (const wPos of [-1, 1]) {
            ...
            wheel.position = transformPos(bikeX, bikeY, bikeZ + wPos * (wheelbase / 2));
