...
export function Signpost({
	...
	useEffect(() => {
		...
		for (let s = 0; s < actualSignCount; s++) {
			...
			const signRotation =
				rotation +
				(rng ? rng.next() * Math.PI * 2 : (s / actualSignCount) * Math.PI * 2);
			...
			arrow.position = new Vector3(
				posX +
					Math.cos(signRotation) * (signLength + postWidth / 2 + arrowSize / 2),
				signY,
				posZ -
					Math.sin(signRotation) * (signLength + postWidth / 2 + arrowSize / 2),
			);
			...
			text.position = new Vector3(
				posX +
					Math.cos(signRotation) * (signLength / 2 + postWidth / 2) -
					Math.sin(signRotation) * 0.012,
				signY,
				posZ -
					Math.sin(signRotation) * (signLength / 2 + postWidth / 2) -
					Math.cos(signRotation) * 0.012,
			);
