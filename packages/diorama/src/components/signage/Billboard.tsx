	...
	useEffect(() => {
		...
		if (type === "standard" || type === "digital") {
			...
			const beam = MeshBuilder.CreateBox(
				`${id}_beam`,
				{
					width: postSpacing + frameThickness * 2,
			...
			beam.position = new Vector3(
				posX,
				posY + postHeight - dims.height / 2 - frameThickness,
				posZ,
			);
			...
			for (const edge of ["top", "bottom", "left", "right"]) {
				...
				frame.position = new Vector3(
					posX +
						Math.sin(rotation) * (displayThickness / 2) +
						Math.cos(rotation) * edgeX,
					posY + postHeight + edgeY,
					posZ +
						Math.cos(rotation) * (displayThickness / 2) -
						Math.sin(rotation) * edgeX,
				);
			...
			if (type === "digital" && powered) {
				...
				for (let i = 0; i < lineCount; i += 2) {
					const line = MeshBuilder.CreateBox(
						`${id}_line_${i}`,
						{
							width: dims.width * 0.99,
							height: dims.height / lineCount,
							depth: 0.01,
						},
						scene,
					);
			...
			for (let i = 0; i < strutCount; i++) {
				const strutX =
					(i - (strutCount - 1) / 2) * (dims.width / (strutCount + 1));
				const strut = MeshBuilder.CreateBox(
					`${id}_strut_${i}`,
					{
						width: frameThickness,
						height: dims.height * 0.8,
						depth: frameThickness * 3,
					},
					scene,
				);
				strut.position = new Vector3(
					posX -
						Math.sin(rotation) * (dims.height * 0.3) +
						Math.cos(rotation) * strutX,
					posY + dims.height * 0.4,
					posZ -
						Math.cos(rotation) * (dims.height * 0.3) -
						Math.sin(rotation) * strutX,
				);
			...
			for (const corner of [
				...
				bracket.position = new Vector3(
					posX -
						Math.sin(rotation) * 0.1 +
						Math.cos(rotation) * (corner[0] * dims.width * 0.4),
					posY + dims.height / 2 + corner[1] * dims.height * 0.4,
					posZ -
						Math.cos(rotation) * 0.1 -
						Math.sin(rotation) * (corner[0] * dims.width * 0.4),
				);
		...
		if (damage > 0.3 && rng) {
			...
			for (let i = 0; i < brokenCount; i++) {
				...
				const displayY =
					type === "standard" || type === "digital"
						? posY + dims.height * 1.5
						: posY + dims.height / 2;

				broken.position = new Vector3(
					posX +
						Math.sin(rotation) * (displayThickness + 0.02) +
						Math.cos(rotation) * offsetX,
					displayY + offsetY,
					posZ +
						Math.cos(rotation) * (displayThickness + 0.02) -
		...
		if (powered && (type === "standard" || type === "rooftop")) {
			...
			for (let i = 0; i < lightCount; i++) {
				...
				const displayY =
					type === "standard" ? posY + dims.height * 2 : posY + dims.height;
				light.position = new Vector3(
					posX +
						Math.sin(rotation) * (displayThickness + 0.2) +
						Math.cos(rotation) * lightX,
					displayY + dims.height / 2 + 0.1,
					posZ +
						Math.cos(rotation) * (displayThickness + 0.2) -
						Math.sin(rotation) * lightX,
				);
