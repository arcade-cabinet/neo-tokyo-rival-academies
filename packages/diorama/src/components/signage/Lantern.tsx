	...
	useEffect(() => {
		...
		const getColor = (colorName: string): Color3 => {
			switch (colorName) {
				...
				case "white":
					return new Color3(0.95, 0.92, 0.85);
				case "orange":
					return new Color3(0.95, 0.5, 0.1);
				case "yellow":
					return new Color3(0.95, 0.9, 0.3);
				case "green":
					return new Color3(0.2, 0.6, 0.3);
				case "blue":
					return new Color3(0.2, 0.4, 0.7);
				default:
					return new Color3(0.85, 0.15, 0.1);
			}
		};
		...
		if (type === "paper") {
			...
			for (let r = 0; r < ringCount; r++) {
				...
				const ringRadius =
					baseRadius * Math.cos((r / (ringCount - 1) - 0.5) * Math.PI * 0.8);
			...
			stem.position = new Vector3(
				posX,
				posY + baseHeight * 0.3 + baseHeight * 0.75,
				posZ,
			);
			...
			const topCap = MeshBuilder.CreateCylinder(
				...
				{
					height: 0.05,
					diameterTop: baseRadius * 0.8,
					diameterBottom: baseRadius * 2.1,
					tessellation: 8,
				},
				scene,
			);
			...
			handle.position = new Vector3(
				posX,
				posY + baseHeight / 2 + baseRadius * 0.75,
				posZ,
			);
		...
		if (hasHanger && type !== "stone") {
			...
			cord.position = new Vector3(
				posX,
				posY +
					(type === "paper" ? baseHeight * 0.8 : baseHeight / 2) +
					cordLength / 2,
				posZ,
			);
