...
export function Flagpole({
	...
	useEffect(() => {
		...
		const getColor = (colorName: string): Color3 => {
			switch (colorName) {
				case "red":
					return new Color3(0.8, 0.15, 0.15);
				case "blue":
					return new Color3(0.15, 0.25, 0.6);
				case "white":
					return new Color3(0.95, 0.95, 0.95);
				case "green":
					return new Color3(0.15, 0.5, 0.2);
				case "yellow":
					return new Color3(0.9, 0.85, 0.2);
				case "orange":
		...
		if (type === "national" || type === "tattered") {
			...
			const flag = MeshBuilder.CreateBox(
				...
				{
					width: 0.02,
					height: flagHeight * (1 - tattering * (rng ? rng.next() : 0.5)),
					depth: flagWidth,
				},
				scene,
			);
			...
			const pennant = MeshBuilder.CreateCylinder(
				`${id}_pennant`,
				{
					height: pennantLength,
					diameterTop: 0,
					diameterBottom: flagHeight * 0.6,
					tessellation: 3,
				},
				scene,
			);
			...
			const sock = MeshBuilder.CreateCylinder(
				`${id}_sock`,
				{
					height: sockLength,
					diameterTop: 0.05,
					diameterBottom: flagHeight * 0.35,
					tessellation: 12,
				},
				scene,
			);
			...
			for (let s = 0; s < 3; s++) {
				const stripe = MeshBuilder.CreateTorus(
					`${id}_sockstripe_${s}`,
					{
						diameter: flagHeight * 0.3 - s * 0.06,
						thickness: 0.02,
						tessellation: 12,
					},
					scene,
				...
				stripe.position = new Vector3(
					sock.position.x -
						Math.cos(rotation) * (sockLength * 0.15 + s * sockLength * 0.2),
					sock.position.y,
					sock.position.z +
						Math.sin(rotation) * (sockLength * 0.15 + s * sockLength * 0.2),
				);
