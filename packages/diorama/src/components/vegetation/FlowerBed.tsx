...
export type FlowerBedCondition =
	| "maintained"
	| "overgrown"
	| "neglected"
	| "flourishing";
		...
		if (type === "circular") {
			...
			for (let ring = 1; ring <= ringCount; ring++) {
				...
				for (let p = 0; p < plantsInRing; p++) {
					const angle =
						(p / plantsInRing) * Math.PI * 2 + (rng ? rng.next() * 0.2 : 0);
					...
					if (rng ? rng.next() < bloomLevel : p % 2 === 0) {
						const flowerMat =
							flowerMats[
								Math.floor(
									(rng ? rng.next() : p / plantsInRing) * flowerMats.length,
								)
							];
			...
			for (let px = 0; px < plantsX; px++) {
				for (let pz = 0; pz < plantsZ; pz++) {
					const localX =
						(px / (plantsX - 1) - 0.5) * (width - 0.15) +
						(rng ? (rng.next() - 0.5) * 0.04 : 0);
					const localZ =
						(pz / (plantsZ - 1) - 0.5) * (depth - 0.15) +
						(rng ? (rng.next() - 0.5) * 0.04 : 0);

					const plantX =
						posX + localX * Math.cos(rotation) - localZ * Math.sin(rotation);
					const plantZ =
						posZ + localX * Math.sin(rotation) + localZ * Math.cos(rotation);
			...
			for (const [side, len, offX, offZ, rotY] of [
				...
				[
					"left",
					depth - plankThickness * 2,
					-width / 2 + plankThickness / 2,
					0,
					Math.PI / 2,
				],
				[
			...
			for (let s = 0; s < stoneCount; s++) {
				const stoneX =
					(rng ? rng.next() - 0.5 : s / stoneCount - 0.5) * width * 0.9;
			...
			for (let p = 0; p < plantCount; p++) {
				const localX =
					(rng ? rng.next() - 0.5 : Math.random() - 0.5) * width * 0.95;
				const localZ =
					(rng ? rng.next() - 0.5 : Math.random() - 0.5) * depth * 0.95;
				...
				if (rng ? rng.next() < bloomLevel * 0.7 : p % 4 === 0) {
					const flowerMat =
						flowerMats[
							Math.floor(
								(rng ? rng.next() : p / plantCount) * flowerMats.length,
							)
						];
			...
			if (condition === "overgrown" || condition === "neglected") {
				...
				for (let w = 0; w < weedCount; w++) {
					...
					const weedWorldX =
						posX + weedX * Math.cos(rotation) - weedZ * Math.sin(rotation);
					const weedWorldZ =
						posZ + weedX * Math.sin(rotation) + weedZ * Math.cos(rotation);
