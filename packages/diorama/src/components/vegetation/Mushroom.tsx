...
export function Mushroom({
	...
	useEffect(() => {
		...
		if (type === "common") {
			...
			stemMat.albedoColor = new Color3(
				0.85 * freshness,
				0.8 * freshness,
				0.75 * freshness,
			);
		...
		if (type === "common") {
			...
			if (!isEdible && rng) {
				...
				for (let s = 0; s < spotCount; s++) {
					...
					const spotY = Math.sqrt(
						Math.max(0, capRadius * capRadius - spotDist * spotDist),
					);

					spot.position = new Vector3(
						posX + spotX,
						posY + stemHeight + spotY * 0.95,
						posZ + spotZ,
					);
			...
			for (let m = 0; m < mushroomCount; m++) {
				const offsetAngle = rng
					? rng.next() * Math.PI * 2
					: (m / mushroomCount) * Math.PI * 2;
			...
			for (let s = 0; s < shelfCount; s++) {
				...
				const shelfY =
					posY + s * 0.05 * size + (rng ? (rng.next() - 0.5) * 0.02 : 0);
			...
			for (let m = 0; m < mushroomCount; m++) {
				...
				if (rng && rng.next() > 0.4) {
					const glowSpotMat = new PBRMaterial(
						`mushroom_glowspot_${id}_${m}`,
						scene,
					);
