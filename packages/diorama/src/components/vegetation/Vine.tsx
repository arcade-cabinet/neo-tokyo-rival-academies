	...
	useEffect(() => {
		...
		for (let s = 0; s < strandCount; s++) {
			...
			for (let seg = 0; seg < segmentCount; seg++) {
				const segmentLength =
					(length / segmentCount) * (0.8 + (rng ? rng.next() * 0.4 : 0.2));
				const segmentThickness =
					type === "tech_cable"
						? 0.015 + (rng ? rng.next() * 0.01 : 0)
						: 0.02 +
							(rng ? rng.next() * 0.015 : 0) * (1 - (seg / segmentCount) * 0.5);
				...
				if (type !== "tech_cable") {
					const leafCount = Math.floor(
						2 + (rng ? rng.next() * 3 : 1) * coverage * densityMultiplier,
					);
					...
					for (let l = 0; l < leafCount; l++) {
						...
						leaf.rotation.x =
							Math.PI / 2 + (rng ? (rng.next() - 0.5) * 0.5 : 0);
		...
		if (type === "wisteria" && hasFlowers && healthFactor > 0.5) {
			...
			for (let c = 0; c < clusterCount; c++) {
				const clusterX = rng
					? (rng.next() - 0.5) * width * 0.7
					: (c / clusterCount - 0.5) * width * 0.7;
				const clusterY = rng
					? rng.next() * length * 0.8
					: (c / clusterCount) * length * 0.7;
		...
		if (condition === "overgrown" && type !== "tech_cable") {
			...
			for (let e = 0; e < extraLeafCount; e++) {
				...
				extraLeaf.rotation.x =
					Math.PI / 2 + (rng ? (rng.next() - 0.5) * 0.6 : 0);
