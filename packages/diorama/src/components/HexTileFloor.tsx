...
import { createAllTileMaterials } from "../materials/tile-materials";
import type { GridBounds } from "../types/grid";
import { TileType } from "../types/tiles";
import {
	applyLeftClipping,
	applyRightClipping,
} from "../utils/clipping-planes";
import { createHexMatrix, HEX_SIZE } from "../utils/hex-grid";
...
export function HexTileFloor({
	...
	useEffect(() => {
		...
		for (const [type, typeTiles] of Object.entries(tilesByType)) {
			...
			instanceMesh.thinInstanceSetBuffer(
				"matrix",
		...
		if (debug) {
			...
			for (const mesh of createdMeshes) {
				if (mesh.material) {
					// biome-ignore lint/suspicious/noExplicitAny: wireframe exists on materials
			...
			// biome-ignore lint/suspicious/noExplicitAny: wireframe exists on materials
			(boundsMaterial as any).wireframe = true;
			// biome-ignore lint/suspicious/noExplicitAny: diffuseColor exists on materials
			(boundsMaterial as any).diffuseColor = new Color3(1, 0, 0);
