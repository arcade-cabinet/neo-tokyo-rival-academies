...
export function TrashCan({
	...
	useEffect(() => {
		...
		if (style === "public") {
			...
			bodyMat.albedoColor = (color ?? new Color3(0.15, 0.35, 0.15)).scale(
				dirtFactor,
			);
			...
			const body = MeshBuilder.CreateCylinder(
				...
				{
					height: bodyHeight,
					diameterTop: bodyRadius * 2.1,
					diameterBottom: bodyRadius * 1.8,
				},
				scene,
			);
			...
			if (hasLid) {
				const lid = MeshBuilder.CreateCylinder(
					...
					{
						height: 0.08,
						diameterTop: bodyRadius * 1.8,
						diameterBottom: bodyRadius * 2.2,
					},
					scene,
				);
				if (lidOpen) {
			...
			bodyMat.albedoColor = (color ?? new Color3(0.2, 0.2, 0.22)).scale(
				dirtFactor,
			);
			...
			for (const side of [-1, 1]) {
				...
				wheel.position = new Vector3(
					posX +
						Math.cos(rotation) * ((side * bodyWidth) / 2) -
						Math.sin(rotation) * (bodyDepth / 2 - 0.1),
					posY + 0.075,
					posZ -
						Math.sin(rotation) * ((side * bodyWidth) / 2) -
			...
			bodyMat.albedoColor = (color ?? new Color3(0.3, 0.35, 0.3)).scale(
				dirtFactor,
			);
			...
			accentMat.albedoColor = (color ?? new Color3(0.1, 0.4, 0.2)).scale(
				dirtFactor,
			);
		...
		if (condition === "overflowing" && rng) {
			...
			const canHeight =
				style === "residential" ? 1.1 : style === "industrial" ? 0.8 : 0.9;
