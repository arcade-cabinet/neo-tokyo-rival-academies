...
export function VendingMachine({
	...
	useEffect(() => {
		...
		glassMat.albedoColor = powered
			? new Color3(0.9, 0.95, 1)
			: new Color3(0.3, 0.3, 0.32);
		if (powered) {
		...
		if (type === "drinks" && powered) {
			...
			for (let row = 0; row < rowCount; row++) {
				for (let col = 0; col < colCount; col++) {
					const canMat = new PBRMaterial(
						`vending_can_${id}_${row}_${col}`,
						scene,
					);
					const canColor = rng
					...
					can.position = new Vector3(
						posX +
							...
							Math.cos(rotation) * ((col - 1) * 0.2),
						posY + height * 0.35 + row * 0.15,
						posZ +
							Math.cos(rotation) * (depth / 2 - 0.1) -
							Math.sin(rotation) * ((col - 1) * 0.2),
					);
					can.material = canMat;
					meshes.push(can);
				}
		...
		coinSlot.position = new Vector3(
			posX +
				Math.sin(rotation) * (depth / 2 + 0.025) +
				Math.cos(rotation) * (width * 0.3),
			posY + height * 0.4,
			posZ +
				Math.cos(rotation) * (depth / 2 + 0.025) -
		...
		if (powered) {
			...
			priceDisplay.position = new Vector3(
				posX +
					Math.sin(rotation) * (depth / 2 + 0.03) +
					Math.cos(rotation) * (width * 0.3),
				posY + height * 0.32,
				posZ +
					Math.cos(rotation) * (depth / 2 + 0.03) -
					Math.sin(rotation) * (width * 0.3),
			);
