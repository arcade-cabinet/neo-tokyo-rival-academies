...
export function Umbrella({
  ...
  useEffect(() => {
    ...
    const umbrellaColor =
      color ??
      (rng ? DEFAULT_COLORS[Math.floor(rng.next() * DEFAULT_COLORS.length)] : DEFAULT_COLORS[2]); // Default black
    ...
    if (state === 'closed') {
      ...
      if (type === 'japanese') {
        ...
        for (let j = 0; j <= 8; j++) {
          ...
          hookPath.push(
            new Vector3(0, -Math.cos(angle) * hookRadius, Math.sin(angle) * hookRadius)
          );
        }
      ...
      const canopyY = state === 'inverted' ? baseY - canopyHeight : baseY + shaftLength - 0.1;
      ...
      for (let i = 0; i < ribCount; i++) {
        ...
        const segCenterX =
          posX + Math.cos(angle + segmentAngle / 2) * (segmentRadius / 3) * Math.cos(tiltAngle);
      ...
      if (type === 'japanese') {
        ...
        handle.position = new Vector3(posX - Math.sin(tiltAngle) * 0.08, baseY - 0.075, posZ);
        handle.material = handleMat;
        meshes.push(handle);
      } else {
        const handleStraight = MeshBuilder.CreateCylinder(
        ...
        for (let j = 0; j <= 10; j++) {
          ...
          openHookPath.push(
            new Vector3(0, -Math.cos(angle) * openHookRadius, Math.sin(angle) * openHookRadius)
          );
        }
        const hookCurve = MeshBuilder.CreateTube(
          `${id}_hookCurve`,
      ...
      for (let w = 0; w < wearCount; w++) {
        ...
        const wearY =
          state === 'closed'
            ? isFlat
              ? baseY + 0.04
              : baseY + shaftLength * 0.5 + (rng.next() - 0.5) * shaftLength * 0.3
