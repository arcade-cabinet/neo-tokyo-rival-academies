	...
	useEffect(() => {
		...
		if (style === "box") {
			...
			const pedestal = MeshBuilder.CreateCylinder(
				...
				{
					height: 0.15 * size,
					diameterTop: diameter * 0.4,
					diameterBottom: diameter * 0.5,
				},
				scene,
			);
			...
			for (const side of ["front", "back", "left", "right"]) {
				...
				if (side === "front" || side === "back") {
					...
					wallX = 0;
					wallZ =
						(side === "front" ? 1 : -1) * (bedDepth / 2 - wallThickness / 2);
				} else {
					wallW = wallThickness;
					wallH = bedHeight;
					wallD = bedDepth - wallThickness * 2;
					wallX =
						(side === "right" ? 1 : -1) * (bedWidth / 2 - wallThickness / 2);
					wallZ = 0;
				}
			...
			const soil = MeshBuilder.CreateBox(
				`${id}_soil`,
				{
					width: bedWidth - wallThickness * 2 - 0.02,
					height: 0.02,
					depth: bedDepth - wallThickness * 2 - 0.02,
				},
				scene,
			);
		...
		if (hasPlants) {
			...
			const planterRadius =
				style === "trough" || style === "raised" ? 0.4 * size : 0.15 * size;
			const baseHeight =
				style === "urn"
					? 0.45 * size
					: style === "hanging"
			...
			for (let p = 0; p < plantCount; p++) {
				const angle = rng
					? rng.next() * Math.PI * 2
					: (p / plantCount) * Math.PI * 2;
				const radius = rng
					? rng.next() * planterRadius * 0.7
					: planterRadius * 0.4;
				const plantHeight =
					(0.15 + (rng ? rng.next() * 0.2 : 0.1)) * size * plantHealth;
			...
			if (plantHealth > 0.7 && rng && rng.next() > 0.5) {
				...
				flowerMat.albedoColor =
					flowerColors[Math.floor(rng.next() * flowerColors.length)];
