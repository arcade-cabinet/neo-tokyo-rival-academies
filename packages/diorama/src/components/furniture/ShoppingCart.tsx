...
export function ShoppingCart({
	...
	useEffect(() => {
		...
		if (type === "standard" || type === "small") {
			...
			back.position = transformPos(
				0,
				basketBottom + basketHeight * 0.35,
				-depth / 2,
			);
			...
			childSeat.position = transformPos(
				0,
				basketBottom + basketHeight - 0.1,
				-depth / 2 + 0.15,
			);
		...
		if (state === "abandoned" && rng) {
			...
			for (let i = 0; i < debrisCount; i++) {
				...
				debris.position = transformPos(
					debrisX,
					basketBottom + debrisSize / 2,
					debrisZ,
				);
		...
		if (state === "stacked") {
			...
			nestedMat.albedoColor = new Color3(0.6, 0.62, 0.65).scale(
				condition * 0.9,
			);
			...
			const nestedOffset =
				type === "standard" ? 0.25 : type === "small" ? 0.2 : 0.15;
			const basketBottom =
				CART_DIMENSIONS[type].wheelSize * 2 + 0.1 + nestedOffset;
			...
			nestedBasket.position = transformPos(
				stackOffset,
				basketBottom + height * 0.175,
				0,
			);
