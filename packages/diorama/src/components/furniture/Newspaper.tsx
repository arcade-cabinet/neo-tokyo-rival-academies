		...
		if (type === "single") {
			...
			if (isTorn && rng) {
				...
				for (let i = 0; i < tearCount; i++) {
					...
					const tearPos =
						(rng.next() - 0.5) *
						(tearEdge === "x" ? PAPER_DEPTH : PAPER_WIDTH) *
						0.8;
					...
					tear.position = new Vector3(
						posX +
							(tearEdge === "z"
								? ((rng.next() > 0.5 ? 1 : -1) * PAPER_WIDTH) / 2
								: tearPos),
						posY + PAPER_HEIGHT / 2,
						posZ +
							(tearEdge === "x"
								? ((rng.next() > 0.5 ? 1 : -1) * PAPER_DEPTH) / 2
								: tearPos),
					);
			...
			for (let i = 0; i < actualCount; i++) {
				...
				paper.material = isMagazine
					? magazineMat
					: createPaperMat(i, isWet && i < actualCount / 3);
				meshes.push(paper);
			}
			...
			for (let i = 0; i < actualCount; i++) {
				const angle = rng
					? rng.next() * Math.PI * 2
					: (i / actualCount) * Math.PI * 2;
				const dist = rng
					? rng.next() * scatterRadius
					: (i / actualCount) * scatterRadius;
			...
			for (let s = 0; s < shelfCount; s++) {
				...
				// Papers on shelf
				const papersOnShelf =
					Math.floor(actualPaperCount / shelfCount) +
					(s === 0 ? actualPaperCount % shelfCount : 0);
			...
			sign.position = new Vector3(
				posX,
				posY + standHeight + 0.08,
				posZ - standDepth / 2 + 0.02,
			);
