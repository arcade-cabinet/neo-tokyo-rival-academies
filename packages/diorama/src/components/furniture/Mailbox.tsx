...
export function Mailbox({
	...
	useEffect(() => {
		...
		if (type === "residential") {
			...
			bodyMat.albedoColor = (color ?? new Color3(0.15, 0.15, 0.17)).scale(
				conditionFactor,
			);
			...
			bodyMat.albedoColor = (color ?? new Color3(0.55, 0.55, 0.58)).scale(
				conditionFactor,
			);
			...
			for (let r = 0; r < rows; r++) {
				for (let c = 0; c < cols; c++) {
					...
					door.position = new Vector3(
						posX +
							Math.cos(rotation) * boxX +
							Math.sin(rotation) * (boxDepth / 2 + 0.01),
						posY + 0.8 + (rows * boxHeight + 0.1) / 2 + boxY,
						posZ -
							Math.sin(rotation) * boxX +
							Math.cos(rotation) * (boxDepth / 2 + 0.01),
					);
					door.rotation.y = rotation;
					door.material = bodyMat;
					meshes.push(door);
					...
					lock.position = new Vector3(
						posX +
							Math.cos(rotation) * (boxX + boxWidth / 3) +
							Math.sin(rotation) * (boxDepth / 2 + 0.025),
						posY + 0.8 + (rows * boxHeight + 0.1) / 2 + boxY,
						posZ -
							Math.sin(rotation) * (boxX + boxWidth / 3) +
							Math.cos(rotation) * (boxDepth / 2 + 0.025),
					);
			...
			bodyMat.albedoColor = (color ?? new Color3(0.1, 0.2, 0.5)).scale(
				conditionFactor,
			);
			...
			bodyMat.albedoColor = (color ?? new Color3(0.8, 0.15, 0.1)).scale(
				conditionFactor,
			);
