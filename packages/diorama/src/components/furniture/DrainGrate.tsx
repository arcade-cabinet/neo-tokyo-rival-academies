	...
	useEffect(() => {
		...
		if (type === "slot") {
			...
			for (let s = 0; s < sectionCount; s++) {
				...
				for (let b = 0; b < barsPerSection; b++) {
					const barZ =
						sectionZ +
						(b - (barsPerSection - 1) / 2) *
							(sectionLength / (barsPerSection + 1));
			...
			lintel.position = new Vector3(posX, posY + inletHeight + 0.025, posZ);
		...
		if (waterFlow > 0 && !isBlocked) {
			...
			if (type === "curb") {
				...
				waterPool.position = new Vector3(
					posX,
					posY + frameDepth + grateThickness + 0.002,
					posZ,
				);
		...
		if (isBlocked && rng) {
			...
			sludge.position = new Vector3(
				posX,
				posY + frameDepth + grateThickness + 0.001,
				posZ,
			);
