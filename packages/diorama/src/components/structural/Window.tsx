...
export function Window({
	...
	useEffect(() => {
		...
		if (type === "single" || type === "fixed") {
			...
			const slideOffset =
				state === "open" ? width * 0.4 : state === "halfopen" ? width * 0.2 : 0;
			...
			fixedGlass.position = new Vector3(
				posX - width / 4,
				posY + height / 2,
				posZ,
			);
			...
			const openAngle =
				state === "open" ? Math.PI / 3 : state === "halfopen" ? Math.PI / 6 : 0;
