...
const CONDITION_FACTORS: Record<
	ConditionType,
	{ rust: number; roughness: number; warp: number }
> = {
		...
		if (type === "louvered") {
			...
			for (let row = 0; row < panelRows; row++) {
				...
				panel.position = new Vector3(
					...
					posY +
						frameThickness * 1.5 +
						row * (panelHeight + frameThickness) +
						panelHeight / 2,
				...
				for (const side of ["top", "bottom", "left", "right"]) {
					...
					const moldingWidth = isHorizontal
					...
					if (side === "bottom")
						moldY -= panelHeight / 2 - frameThickness * 0.5;
					if (side === "left")
						moldX +=
							Math.cos(rotation + openAngle) *
					...
					if (side === "right")
						moldX +=
							Math.cos(rotation + openAngle) * (width / 2 - frameThickness * 2);
			...
			panel.position = new Vector3(
				posX +
					Math.sin(rotation) *
						((Math.sin(bahamaAngle) * height) / 2 + frameThickness / 2),
			...
			for (let i = 0; i < louverCount; i++) {
				...
				louver.position = new Vector3(
					posX +
						Math.sin(rotation) *
							(Math.sin(bahamaAngle) * (1 - louverY) * height + frameThickness),
