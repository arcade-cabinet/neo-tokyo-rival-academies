...
export function FireEscape({
	...
	useEffect(() => {
		...
		for (let floor = 0; floor < floors; floor++) {
			...
			const zigzagOffset =
				style === "zigzag" ? (floor % 2 === 0 ? 0 : platformWidth) : 0;
			...
			platform.position = new Vector3(posX + zigzagOffset, platformY, posZ);
			...
			for (const side of [-1, 1]) {
				...
				const needsRailing =
					style !== "zigzag" || (floor % 2 === 0 ? side === 1 : side === -1);
			...
			if (floor < floors - 1) {
				...
				for (const side of [-1, 1]) {
					const stringerLength = Math.sqrt(
						floorHeight ** 2 + (platformWidth * 0.8) ** 2,
					);
					...
					stringer.position = new Vector3(
						posX +
							zigzagOffset +
							stairDirection * platformWidth * 0.4 +
							(side * stairWidth) / 2,
						platformY + floorHeight / 2,
						posZ - platformDepth / 2 + 0.15,
					);
					stringer.rotation.z =
						Math.atan2(floorHeight, platformWidth * 0.8) * stairDirection;
				...
				for (let step = 0; step < stairCount; step++) {
					...
					const stepX =
						stairDirection * (step / stairCount) * platformWidth * 0.8;
