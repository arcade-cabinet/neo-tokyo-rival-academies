...
export function Fence({
	...
	useEffect(() => {
		...
		if (style === "chainlink") {
			...
			panelMat.albedoColor = new Color3(0.45, 0.35, 0.22).scale(
				conditionFactor,
			);
			panelMat.metallic = 0;
		...
		const postRadius =
			style === "wooden" ? 0.06 : style === "bamboo" ? 0.04 : 0.04;
		...
		if (style === "chainlink") {
			...
			for (let p = 0; p < poleCount; p++) {
				...
				const poleHeight =
					height - 0.05 + (rng ? (rng.next() - 0.5) * 0.15 : 0);
				...
				const pole = MeshBuilder.CreateCylinder(
					...
					{
						height: poleHeight,
						diameter: 0.03 + (rng ? rng.next() * 0.01 : 0),
					},
					scene,
				);
		...
		if (condition === "overgrown" && rng) {
			...
			for (let v = 0; v < vineCount; v++) {
				...
				const vine = MeshBuilder.CreateBox(
					`${id}_vine_${v}`,
					{
						width: 0.2 + rng.next() * 0.3,
						height: height * (0.3 + rng.next() * 0.5),
						depth: 0.05,
					},
					scene,
				);
