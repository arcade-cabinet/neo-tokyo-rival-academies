...
const MATERIAL_COLORS: Record<
  DoorMaterial,
  { base: Color3; metallic: number; roughness: number; alpha?: number }
> = {
...
export function Door({
  ...
  useEffect(() => {
    ...
    if (type === 'single') {
      ...
      const shutterHeight =
        state === 'closed' ? height : state === 'halfopen' ? height / 2 : height * 0.1;
      ...
      // Rod at top
      const rod = MeshBuilder.CreateCylinder(`${id}_rod`, { height: width, diameter: 0.03 }, scene);
