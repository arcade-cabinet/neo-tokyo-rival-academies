...
const CONDITION_FACTORS: Record<
	ConditionType,
	{ rust: number; roughness: number; soot: number }
> = {
...
export function Chimney({
	...
	useEffect(() => {
		...
		if (type === "brick") {
			...
			mortarMat.albedoColor = new Color3(0.6, 0.58, 0.55).scale(
				1 - conditionFactor.soot * 0.3,
			);
			...
			for (const side of [-1, 1]) {
				...
				rail.position = new Vector3(
					posX +
						Math.cos(rotation) * (side * 0.12) +
						Math.sin(rotation) * (width / 2 + 0.05),
					posY + height * 0.45,
					posZ -
						Math.sin(rotation) * (side * 0.12) +
			...
			corbel.position = new Vector3(
				posX,
				posY + height * 0.75 + height * 0.04,
				posZ,
			);
			...
			upperStack.position = new Vector3(
				posX,
				posY + height * 0.83 + height * 0.1,
				posZ,
			);
		...
		if (hasCap) {
			...
			const capY =
				posY + height + (type === "decorative" ? height * 0.2 : 0.15);
			const capWidth =
				type === "brick"
					? width * 0.7
					: type === "decorative"
			...
			capRoof.rotation.y =
				type === "brick" || type === "decorative" ? rotation + Math.PI / 4 : 0;
