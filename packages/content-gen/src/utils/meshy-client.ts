...
export class MeshyClient {
  ...
  async stream(path: string, timeoutMs?: number): Promise<unknown> {
    ...
    // @ts-ignore - NodeJS ReadableStream vs Web ReadableStream mismatch handling
    const reader = response.body.getReader ? response.body.getReader() : null; 
    
    // Node Environment Fallback if getReader is missing (node-fetch vs native fetch)
    if (!reader) {
        // Simple polling fallback or assume it's not a stream in Node without extra logic
        // For now, let's assume we are running in Node 20+ which has native fetch
        throw new Error("Response body does not support getReader()");
    }
