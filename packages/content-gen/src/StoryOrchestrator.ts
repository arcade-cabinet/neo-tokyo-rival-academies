...
async function batchProcess<T, R>(items: T[], batchSize: number, processor: (item: T) => Promise<R>): Promise<R[]> {
  ...
  for (let i = 0; i < items.length; i += batchSize) {
    ...
    try {
      ...
      // Continue with partial results or retry logic could go here
    }
...
async function orchestrate() {
  ...
  // Prepare Meso Tasks
  const mesoTasks = [];
  // Safety check for undefined macroStory structure
  ...
  // Batch size 2 to be very safe with experimental model limits
  ...
  const outputPath = path.resolve(__dirname, '../../../packages/game/src/data/generated_jrpg.json');
