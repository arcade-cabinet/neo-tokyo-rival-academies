...
export function Pipe({
	...
	useEffect(() => {
		...
		if (material === "copper") {
			pipeColor = new Color3(0.72 - corrosionVariation, 0.45 - corrosionVariation * 2, 0.2 - corrosionVariation);
			...
			pipeColor = new Color3(0.45 - corrosionVariation, 0.47 - corrosionVariation, 0.5 - corrosionVariation);
		}
		...
		if (valve) {
			...
			const perpendicular = Vector3.Cross(direction.normalize(), new Vector3(1, 0, 0));
			if (perpendicular.length() < 0.1) {
				perpendicular.copyFrom(Vector3.Cross(direction.normalize(), new Vector3(0, 0, 1)));
			}
	...
	}, [scene, id, start, end, material, size, flanges, valve, valvePosition, insulated, corrosion, seed]);
