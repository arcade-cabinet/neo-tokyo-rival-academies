...
export type BenchStyle = "park" | "modern" | "japanese" | "concrete" | "bus_stop" | "stadium";
...
export function Bench({
	...
	useEffect(() => {
		...
		const conditionFactor = condition === "new" ? 1 : condition === "weathered" ? 0.85 : condition === "damaged" ? 0.7 : 0.6;
		...
		if (style === "park") {
			...
			seatMat.albedoColor = new Color3(0.45 * conditionFactor, 0.32 * conditionFactor, 0.18 * conditionFactor);
			seatMat.metallic = 0;
			...
			for (const side of [-1, 1]) {
				...
				frontLeg.position = new Vector3(
					posX + Math.cos(rotation) * legX + Math.sin(rotation) * (seatDepth / 2 - 0.05),
					posY + seatHeight / 2,
					posZ - Math.sin(rotation) * legX + Math.cos(rotation) * (seatDepth / 2 - 0.05)
				);
				...
				const backLeg = MeshBuilder.CreateBox(
					`${id}_backLeg_${side}`,
					{ width: 0.05, height: hasBackrest ? seatHeight + backHeight : seatHeight, depth: 0.05 },
					scene
				);
			...
			if (hasBackrest) {
				for (let i = 0; i < slatCount; i++) {
					...
					slat.position = new Vector3(
						posX + Math.cos(rotation) * slatX - Math.sin(rotation) * (seatDepth / 2 - 0.02),
						posY + seatHeight + backHeight / 2,
						posZ - Math.sin(rotation) * slatX - Math.cos(rotation) * (seatDepth / 2 - 0.02)
					);
					slat.rotation.y = rotation;
					slat.material = seatMat;
					meshes.push(slat);
			...
			seatMat.albedoColor = new Color3(0.55 * conditionFactor, 0.42 * conditionFactor, 0.25 * conditionFactor);
			seatMat.metallic = 0;
			...
			seatMat.albedoColor = new Color3(0.55 * conditionFactor, 0.53 * conditionFactor, 0.5 * conditionFactor);
			seatMat.metallic = 0;
			...
			frameMat.albedoColor = new Color3(0.25, 0.25, 0.28).scale(conditionFactor);
			frameMat.metallic = 0.9;
		...
		// Armrests
		if (hasArmrests && (style === "park" || style === "modern" || style === "bus_stop")) {
			...
			for (const side of [-1, 1]) {
				...
				armSupport.position = new Vector3(
					posX + Math.cos(rotation) * armX + Math.sin(rotation) * (seatDepth / 2 - 0.1),
					posY + seatHeight + 0.1,
					posZ - Math.sin(rotation) * armX + Math.cos(rotation) * (seatDepth / 2 - 0.1)
				);
	...
	}, [scene, id, posX, posY, posZ, style, length, hasBackrest, hasArmrests, condition, rotation, seed]);
