...
export function Newspaper({
	...
	useEffect(() => {
		...
		if (type === "single") {
			...
			headline.position = new Vector3(posX, posY + PAPER_HEIGHT + 0.0003, posZ + PAPER_DEPTH * 0.38);
			...
			if (actualCount > 3) {
				...
				const band = MeshBuilder.CreateTorus(
					`${id}_band`,
					{ diameter: Math.max(PAPER_WIDTH, PAPER_DEPTH) + 0.02, thickness: 0.005, tessellation: 24 },
					scene
				);
			...
			for (let s = 0; s < shelfCount; s++) {
				...
				// Papers on shelf
				const papersOnShelf = Math.floor(actualPaperCount / shelfCount) + (s === 0 ? actualPaperCount % shelfCount : 0);
				...
				for (let p = 0; p < papersOnShelf; p++) {
					...
					const paper = MeshBuilder.CreateBox(
						...
						{ width: PAPER_WIDTH * 0.9, height: PAPER_HEIGHT * 3, depth: PAPER_DEPTH * 0.9 },
						scene
					);
