...
export function Pier({
	...
	useEffect(() => {
		...
		pilingMat.albedoColor = style === "wooden"
			? new Color3(0.3 - weatherVariation, 0.25 - weatherVariation, 0.15 - weatherVariation)
		...
		if (style === "wooden") {
			...
			for (let i = 0; i < plankCount; i++) {
				...
				const plank = MeshBuilder.CreateBox(
					`${id}_plank_${i}`,
					{ width: plankWidth - plankGap, height: deckThickness, depth: length },
					scene
				);
			...
			for (let i = 0; i < pilingCountLength; i++) {
				for (const side of [-1, 1]) {
					...
					piling.position = new Vector3(
						posX + Math.cos(rotation + Math.PI / 2) * (side * (width / 2 - 0.15)) + Math.cos(rotation) * pilingZ,
						posY - pilingDepth / 2 - deckThickness,
						posZ + pilingZ + Math.sin(rotation + Math.PI / 2) * (side * (width / 2 - 0.15))
					);
		...
		if (cleats) {
			...
			for (let i = 0; i < cleatCount; i++) {
				for (const side of [-1, 1]) {
					const cleatZ = (i + 0.5) * (length / cleatCount) * cleatSpacing / (length / cleatCount);
					...
					cleatBase.position = new Vector3(
						posX + Math.cos(rotation + Math.PI / 2) * (side * (width / 2 - 0.3)) + Math.cos(rotation) * cleatZ,
						deckHeight + 0.075,
						posZ + cleatZ + Math.sin(rotation + Math.PI / 2) * (side * (width / 2 - 0.3))
					);
		...
		if (lights) {
			...
			for (let i = 0; i < lightCount; i++) {
				...
				pole.position = new Vector3(
					posX + Math.cos(rotation + Math.PI / 2) * (width / 2 - 0.2) + Math.cos(rotation) * lightZ,
					deckHeight + 1.25,
	...
	}, [scene, id, posX, posY, posZ, length, width, style, waterLevel, cleats, lights, rotation, weathering, seed]);
