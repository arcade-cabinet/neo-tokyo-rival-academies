...
export function Dumpster({
	...
	useEffect(() => {
		...
		const conditionFactor = condition === "new" ? 1 : condition === "used" ? 0.85 : condition === "rusted" ? 0.65 : 0.55;
		const rustAmount = condition === "rusted" ? 0.3 : condition === "damaged" ? 0.4 : 0;
		...
		if (type === "standard" || type === "frontload") {
			...
			for (let r = 0; r < ribCount; r++) {
				...
				for (const side of [-1, 1]) {
					...
					rib.position = new Vector3(
						posX + Math.cos(rotation) * ribX + Math.sin(rotation) * (side * bodyDepth / 2),
						posY + bodyHeight / 2 + 0.1,
						posZ - Math.sin(rotation) * ribX + Math.cos(rotation) * (side * bodyDepth / 2)
					);
			...
			if (type === "frontload") {
				for (const side of [-1, 1]) {
					...
					pocket.position = new Vector3(
						posX + Math.cos(rotation) * (side * bodyWidth * 0.3) + Math.sin(rotation) * (bodyDepth / 2 + 0.1),
						posY + bodyHeight * 0.7,
						posZ - Math.sin(rotation) * (side * bodyWidth * 0.3) + Math.cos(rotation) * (bodyDepth / 2 + 0.1)
					);
			...
			for (const wx of [-1, 1]) {
				for (const wz of [-1, 1]) {
					...
					wheel.position = new Vector3(
						posX + Math.cos(rotation) * (wx * (bodyWidth / 2 - 0.15)) - Math.sin(rotation) * (wz * (bodyDepth / 2 - 0.1)),
						posY + 0.075,
						posZ - Math.sin(rotation) * (wx * (bodyWidth / 2 - 0.15)) - Math.cos(rotation) * (wz * (bodyDepth / 2 - 0.1))
					);
			...
			panel.position = new Vector3(
				posX + Math.cos(rotation) * (bodyWidth / 2 + 0.05) + Math.sin(rotation) * (bodyDepth / 3),
				posY + bodyHeight * 0.5,
				posZ - Math.sin(rotation) * (bodyWidth / 2 + 0.05) + Math.cos(rotation) * (bodyDepth / 3)
			);
		...
		if (condition === "damaged" && rng) {
			...
			for (let d = 0; d < dentCount; d++) {
				...
				const dent = MeshBuilder.CreateBox(
					`${id}_dent_${d}`,
					{ width: 0.2 + rng.next() * 0.2, height: 0.15 + rng.next() * 0.2, depth: 0.02 },
					scene
				);
				const dentSide = rng.next() > 0.5 ? 1 : -1;
				const bodyDepthActual = type === "rolloff" ? 5 * size : 1.5 * size;
				dent.position = new Vector3(
					posX + Math.sin(rotation) * ((rng.next() - 0.5) * bodyDepthActual * 0.8) + Math.cos(rotation) * (dentSide * 0.9 * size),
					posY + 0.3 + rng.next() * 0.6 * size,
					posZ + Math.cos(rotation) * ((rng.next() - 0.5) * bodyDepthActual * 0.8) - Math.sin(rotation) * (dentSide * 0.9 * size)
				);
	...
	}, [scene, id, posX, posY, posZ, type, size, condition, fillLevel, lidOpen, color, rotation, seed]);
