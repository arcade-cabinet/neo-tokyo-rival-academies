		...
		if (type === "louvered") {
			...
			for (const side of [-1, 1]) {
				const sideRail = MeshBuilder.CreateBox(
					`${id}_rail_${side}`,
					{ width: frameThickness, height: height - frameThickness * 2, depth: frameThickness },
					scene
				);
				sideRail.position = new Vector3(
					posX + Math.cos(rotation + openAngle) * (side * (width / 2 - frameThickness / 2)) +
						Math.sin(rotation + openAngle) * frameThickness / 2,
					posY + height / 2,
					posZ - Math.sin(rotation + openAngle) * (side * (width / 2 - frameThickness / 2)) +
			...
			for (let row = 0; row < panelRows; row++) {
				...
				for (const side of ["top", "bottom", "left", "right"]) {
					...
					const moldingHeight = isHorizontal ? frameThickness * 0.4 : panelHeight - frameThickness * 1.5;

					const molding = MeshBuilder.CreateBox(
						`${id}_molding_${row}_${side}`,
						{ width: moldingWidth, height: moldingHeight, depth: frameThickness * 0.3 },
						scene
					);

					let moldX = posX;
					let moldY = posY + frameThickness * 1.5 + row * (panelHeight + frameThickness) + panelHeight / 2;
					if (side === "top") moldY += panelHeight / 2 - frameThickness * 0.5;
					if (side === "bottom") moldY -= panelHeight / 2 - frameThickness * 0.5;
					if (side === "left") moldX += Math.cos(rotation + openAngle) * (-(width / 2 - frameThickness * 2));
					if (side === "right") moldX += Math.cos(rotation + openAngle) * (width / 2 - frameThickness * 2);

					molding.position = new Vector3(
						moldX + Math.sin(rotation + openAngle) * frameThickness * 0.6,
						moldY,
						posZ + Math.cos(rotation + openAngle) * frameThickness * 0.6 +
							(side === "left" ? -Math.sin(rotation + openAngle) * (width / 2 - frameThickness * 2) :
							 side === "right" ? -Math.sin(rotation + openAngle) * -(width / 2 - frameThickness * 2) : 0)
					);
			...
			const bahamaAngle = state === "open" ? Math.PI / 3 :
				state === "half" ? Math.PI / 5 :
				state === "broken" ? Math.PI / 4 + (rng ? rng.next() * 0.3 : 0.1) :
				Math.PI / 8;
			...
			for (let i = 0; i < louverCount; i++) {
				...
				louver.position = new Vector3(
					posX + Math.sin(rotation) * (Math.sin(bahamaAngle) * (1 - louverY) * height + frameThickness),
					posY + height - Math.cos(bahamaAngle) * (1 - louverY) * height,
					posZ + Math.cos(rotation) * (Math.sin(bahamaAngle) * (1 - louverY) * height + frameThickness)
				);
			...
			for (const side of [-1, 1]) {
				const arm = MeshBuilder.CreateBox(
					`${id}_arm_${side}`,
					{ width: 0.015, height: Math.sin(bahamaAngle) * height * 0.8, depth: 0.015 },
					scene
				);
				arm.position = new Vector3(
					posX + Math.cos(rotation) * (side * (width / 2 - 0.05)) +
						Math.sin(rotation) * (Math.sin(bahamaAngle) * height * 0.4 + 0.05),
					posY + height * 0.7,
					posZ - Math.sin(rotation) * (side * (width / 2 - 0.05)) +
			...
			for (let i = 0; i < slatCount; i++) {
				...
				const warp = condition === "damaged" && rng
					? (rng.next() - 0.5) * conditionFactor.warp * 5
					: 0;
				...
				const lip = MeshBuilder.CreateBox(
					`${id}_lip_${i}`,
					{ width: width - 0.02, height: slatHeight * 0.2, depth: slatDepth * 0.6 },
					scene
				);
			...
			const cover = MeshBuilder.CreateBox(
				`${id}_cover`,
				{ width: width + 0.12, height: housingRadius * 2.2, depth: housingRadius * 2.5 },
				scene
			);
			...
			if (visibleHeight > 0.1) {
				const handleBar = MeshBuilder.CreateBox(
					`${id}_handle`,
					{ width: width - 0.02, height: slatHeight * 1.2, depth: slatDepth * 1.5 },
					scene
				);
