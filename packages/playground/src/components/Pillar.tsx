...
const MATERIAL_COLORS: Record<PillarMaterial, { base: Color3; metallic: number; roughness: number }> = {
...
export function Pillar({
	...
	useEffect(() => {
		...
		const shaftHeight = height - (base ? baseHeight : 0) - (capital ? capitalHeight : 0);
		...
		if (shape === "square") {
			...
			for (const yOffset of [-shaftHeight / 2 + flangeThickness / 2, shaftHeight / 2 - flangeThickness / 2]) {
		...
		if (capital) {
			...
			const capitalMesh = shape === "round"
				? MeshBuilder.CreateCylinder(
					`${id}_capital`,
					{ height: capitalHeight, diameterTop: width * 1.4, diameterBottom: width },
					scene
				)
				: MeshBuilder.CreateBox(
					`${id}_capital`,
		...
		if (weathering > 0.3 && rng) {
			...
			for (let i = 0; i < streakCount; i++) {
				...
				const streak = MeshBuilder.CreateBox(
					`${id}_streak_${i}`,
					{ width: width * 0.3 * rng.next() + 0.1, height: streakHeight, depth: 0.01 },
					scene
				);
	...
	}, [scene, id, posX, posY, posZ, height, width, shape, material, capital, base, weathering, seed]);
