...
export function Rope({
		...
		if (style === "straight") {
			...
			for (let i = 0; i < points.length - 1; i++) {
				...
				if (axis.length() > 0.001) {
					...
					segment.rotation = rotMatrix.getRotationMatrix().toEulerAngles() as any;
				}
			...
			if (type === "chain") {
				...
				for (let l = 0; l < linkCount; l++) {
					...
					const link = MeshBuilder.CreateTorus(
						`${id}_link_${l}`,
						{ diameter: thickness * 2, thickness: thickness * 0.4, tessellation: 8 },
						scene
					);
		...
		if (type === "hemp" && condition < 0.6) {
			...
			for (let f = 0; f < frayCount; f++) {
				...
				const fray = MeshBuilder.CreateCylinder(
					`${id}_fray_${f}`,
					{ height: frayLen, diameterTop: 0.001, diameterBottom: thickness * 0.3 },
					scene
				);
	...
	}, [scene, id, posX, posY, posZ, type, style, length, thickness, endPosition, sag, condition, rotation, seed]);
