...
export function FlowerBed({
	...
	useEffect(() => {
		...
		const healthFactor =
			condition === "maintained" ? 0.9 :
			condition === "overgrown" ? 0.75 :
			condition === "neglected" ? 0.5 :
			1.0; // flourishing

		const densityMultiplier =
			condition === "maintained" ? 1.0 :
			condition === "overgrown" ? 1.5 :
			condition === "neglected" ? 0.6 :
			1.3; // flourishing
		...
		if (type === "circular") {
			...
			for (let ring = 1; ring <= ringCount; ring++) {
				...
				for (let p = 0; p < plantsInRing; p++) {
					...
					if (rng ? rng.next() < bloomLevel : p % 2 === 0) {
						const flowerMat = flowerMats[Math.floor((rng ? rng.next() : p / plantsInRing) * flowerMats.length)];
			...
			for (let px = 0; px < plantsX; px++) {
				for (let pz = 0; pz < plantsZ; pz++) {
					const localX = (px / (plantsX - 1) - 0.5) * (width - 0.15) + (rng ? (rng.next() - 0.5) * 0.04 : 0);
					const localZ = (pz / (plantsZ - 1) - 0.5) * (depth - 0.15) + (rng ? (rng.next() - 0.5) * 0.04 : 0);
					...
					if (rng ? rng.next() < bloomLevel : (px + pz) % 2 === 0) {
						const flowerMat = flowerMats[Math.floor((rng ? rng.next() : (px * plantsZ + pz) / (plantsX * plantsZ)) * flowerMats.length)];
			...
			for (let px = 0; px < plantsX; px++) {
				for (let pz = 0; pz < plantsZ; pz++) {
					const localX = (px / Math.max(1, plantsX - 1) - 0.5) * (width - 0.2) + (rng ? (rng.next() - 0.5) * 0.05 : 0);
					const localZ = (pz / Math.max(1, plantsZ - 1) - 0.5) * (depth - 0.2) + (rng ? (rng.next() - 0.5) * 0.05 : 0);
			...
			for (let p = 0; p < plantCount; p++) {
				...
				if (rng ? rng.next() < bloomLevel * 0.7 : p % 4 === 0) {
					const flowerMat = flowerMats[Math.floor((rng ? rng.next() : p / plantCount) * flowerMats.length)];
		...
		if (condition === "flourishing" && rng) {
			...
			for (let i = 0; i < insectCount; i++) {
				...
				const insect = MeshBuilder.CreateSphere(
					`${id}_insect_${i}`,
					{ diameter: 0.01 },
					scene
				);
	...
	}, [scene, id, posX, posY, posZ, rotation, type, condition, width, depth, flowerType, bloomLevel, seed]);
