...
export type BollardType = "fixed" | "removable" | "flexible" | "decorative" | "lit";
...
export function BollardPost({
	...
	useEffect(() => {
		...
		if (type === "fixed") {
			...
			const socket = MeshBuilder.CreateCylinder(
				`${id}_socket`,
				{ height: 0.05, diameterTop: diameter * 1.4, diameterBottom: diameter * 1.5 },
				scene
			);
			...
			const cap = MeshBuilder.CreateCylinder(
				`${id}_cap`,
				{ height: 0.04, diameterTop: diameter * 0.7, diameterBottom: diameter * 0.95 },
				scene
			);
			...
			// Reflective stripes
			accentMat.albedoColor = new Color3(0.95, 0.95, 0.95).scale(conditionFactor);
			...
			const base = MeshBuilder.CreateCylinder(
				`${id}_base`,
				{ height: 0.1, diameterTop: diameter * 0.9, diameterBottom: diameter * 1.3, tessellation: 8 },
				scene
			);
			...
			shaft.position = new Vector3(posX, posY + 0.15 + (height * 0.6) / 2, posZ);
			...
			const finialBase = MeshBuilder.CreateCylinder(
				`${id}_finialBase`,
				{ height: 0.08, diameterTop: diameter * 0.6, diameterBottom: diameter * 0.85 },
				scene
			);
			finialBase.position = new Vector3(posX, posY + 0.15 + height * 0.6 + 0.04, posZ);
			...
			finialBall.position = new Vector3(posX, posY + 0.15 + height * 0.6 + 0.1 + diameter * 0.25, posZ);
	...
	}, [scene, id, posX, posY, posZ, type, height, diameter, hasLight, lightColor, lightOn, condition, rotation, seed]);
