...
export function Roof({
	...
	useEffect(() => {
		...
		switch (style) {
			...
			case "glass": {
				...
				for (let x = 0; x <= panelCountX; x++) {
					...
					frameX.position = new Vector3(
						-size.width / 2 + x * panelWidth,
						0,
						0,
					);
				...
				for (let z = 0; z <= panelCountZ; z++) {
					...
					frameZ.position = new Vector3(
						0,
						0,
						-size.depth / 2 + z * panelDepth,
					);
		...
		if (supportBeams) {
			...
			const actualBeamCount =
				beamCount ?? Math.max(2, Math.floor(size.width / 3));
		...
		if (equipment) {
			...
			for (let i = 0; i < equipCount; i++) {
				...
				const eqX =
					(nextRandom() - 0.5) * (size.width - safeMargin * 2);
				const eqZ =
					(nextRandom() - 0.5) * (size.depth - safeMargin * 2);
				...
				switch (eqType) {
					case 0: {
						...
						ac.position = new Vector3(
							eqX,
							thickness / 2 + acHeight / 2,
							eqZ,
						);
