...
const UMBRELLA_SPECS: Record<UmbrellaType, { radius: number; shaftLength: number; ribCount: number }> = {
...
export function Umbrella({
	...
	useEffect(() => {
		...
		const secondaryColor = type === "golf"
		...
		if (state === "closed") {
			...
			if (isFlat) {
				...
				furledCanopy.position = new Vector3(posX, baseY + shaftLength * 0.55, posZ);
			}
			...
			for (let i = 0; i < ribCount; i++) {
				...
				const segmentRadius = state === "broken" && rng && rng.next() > 0.7
				...
				const segCenterX = posX + Math.cos(angle + segmentAngle / 2) * (segmentRadius / 3) * Math.cos(tiltAngle);
				const segCenterZ = posZ + Math.sin(angle + segmentAngle / 2) * (segmentRadius / 3);

				segment.position = new Vector3(
					segCenterX,
					canopyY + (state === "inverted" ? canopyHeight * 0.5 : -canopyHeight * 0.5),
					segCenterZ
				);
				segment.rotation.x = state === "inverted" ? Math.PI / 2 + 0.3 : -Math.PI / 2 + 0.3;
		...
		if (type === "japanese" && state === "open") {
			...
			for (let r = 0; r < ringCount; r++) {
				...
				const ring = MeshBuilder.CreateTorus(
					`${id}_decorRing_${r}`,
					{ diameter: ringRadius * 2, thickness: 0.003, tessellation: ribCount },
					scene
				);
		...
		if (condition < 0.5 && rng) {
			...
			for (let w = 0; w < wearCount; w++) {
				...
				const wearY = state === "closed"
					? (isFlat ? baseY + 0.04 : baseY + shaftLength * 0.5 + (rng.next() - 0.5) * shaftLength * 0.3)
					: baseY + shaftLength - 0.1;
	...
	}, [scene, id, posX, posY, posZ, type, state, color, condition, rotation, seed]);
