...
export function Tarp({
		...
		if (type === "flat") {
			...
			for (const side of [-1, 1]) {
				const pole = MeshBuilder.CreateCylinder(
					...
					{ height: awningHeight - depth * Math.sin(slopeAngle), diameter: 0.02 },
					scene
				);
				pole.position = new Vector3(
	...
	}, [scene, id, posX, posY, posZ, type, width, depth, color, hasHoles, isTied, condition, rotation, seed]);
