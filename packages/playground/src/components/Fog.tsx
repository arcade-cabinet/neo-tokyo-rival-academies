...
export function Fog({
	...
	useEffect(() => {
		...
		if (type === "ground") {
			...
			for (let l = 0; l < layerCount; l++) {
				...
				layer.position = new Vector3(posX, layerY + height / layerCount / 2, posZ);
			...
			for (let c = 0; c < cloudCount; c++) {
				...
				const cloudSize = (1 + (rng ? rng.next() : 0.5)) * Math.min(width, depth) * 0.15;
			...
			for (let p = 0; p < patchCount; p++) {
				...
				const patchSize = (2 + (rng ? rng.next() * 2 : 1)) * Math.min(width, depth) * 0.1;
				...
				for (let ws = 0; ws < wispCount; ws++) {
					...
					const wisp = MeshBuilder.CreateCylinder(
						`${id}_wisp_${p}_${ws}`,
						{ height: height * 0.5, diameterTop: 0.05, diameterBottom: patchSize * 0.3 },
						scene
					);
	...
	}, [scene, id, posX, posY, posZ, type, width, height, depth, density, tint, seed]);
