...
export function Tree({
	...
	useEffect(() => {
		...
		trunkMat.albedoColor = type === "palm"
		...
		const trunkHeight = (type === "palm" ? 4 : type === "bonsai" ? 0.4 : 2.5) * scale;
		const trunkRadius = (type === "palm" ? 0.15 : type === "bonsai" ? 0.05 : 0.15) * scale;
		...
		if (type === "deciduous" || type === "dead") {
			...
			if (type !== "dead" || health > 0) {
				...
				for (let i = 0; i < branchCount; i++) {
					const branchAngle = (i / branchCount) * Math.PI * 2 + (rng ? rng.next() * 0.5 : 0);
					const branchLength = (0.8 + (rng ? rng.next() * 0.4 : 0)) * scale;

					const branch = MeshBuilder.CreateCylinder(
						`${id}_branch_${i}`,
						{ height: branchLength, diameterTop: 0.02 * scale, diameterBottom: 0.05 * scale },
						scene
					);
					branch.position = new Vector3(
						posX + Math.cos(branchAngle) * branchLength * 0.4,
						posY + trunkHeight * 0.7 + (rng ? rng.next() * trunkHeight * 0.2 : 0),
			...
			if (type !== "dead" && health > 0.3) {
				...
				for (let i = 0; i < clusterCount; i++) {
					...
					const angle = rng ? rng.next() * Math.PI * 2 : (i / clusterCount) * Math.PI * 2;
			...
			const trunk = MeshBuilder.CreateCylinder(
				`${id}_trunk`,
				{ height: trunkHeight, diameterTop: trunkRadius * 0.5, diameterBottom: trunkRadius },
				scene
			);
			trunk.position = new Vector3(posX, posY + trunkHeight / 2, posZ);
			trunk.material = trunkMat;
			...
			for (let i = 0; i < layerCount; i++) {
				const layerY = posY + trunkHeight * 0.4 + i * (trunkHeight * 0.5 / layerCount);
			...
			const trunk = MeshBuilder.CreateCylinder(
				`${id}_trunk`,
				{ height: trunkHeight, diameterTop: trunkRadius * 0.8, diameterBottom: trunkRadius },
				scene
			);
			trunk.position = new Vector3(posX, posY + trunkHeight / 2, posZ);
			trunk.material = trunkMat;
			...
			for (let i = 0; i < bandCount; i++) {
				...
				band.position = new Vector3(posX, posY + (i + 0.5) * (trunkHeight / bandCount), posZ);
			...
			for (let i = 0; i < frondCount; i++) {
				const frondAngle = (i / frondCount) * Math.PI * 2 + (rng ? rng.next() * 0.3 : 0);
			...
			const trunk = MeshBuilder.CreateCylinder(
				`${id}_trunk`,
				{ height: trunkHeight, diameterTop: trunkRadius * 0.7, diameterBottom: trunkRadius },
				scene
			);
			trunk.position = new Vector3(posX, posY + trunkHeight / 2, posZ);
			...
			const pot = MeshBuilder.CreateCylinder(
				`${id}_pot`,
				{ height: 0.15 * scale, diameterTop: 0.3 * scale, diameterBottom: 0.25 * scale },
				scene
			);
