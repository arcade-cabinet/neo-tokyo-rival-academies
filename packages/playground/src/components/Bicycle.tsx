...
export type BicycleType = "city" | "mountain" | "racing" | "mamachari" | "folding" | "vintage";
...
export function Bicycle({
	...
	useEffect(() => {
		...
		switch (type) {
			case "city":
				frameMat.albedoColor = new Color3(0.2, 0.35, 0.5).scale(conditionFactor);
				break;
			case "mountain":
				frameMat.albedoColor = new Color3(0.15, 0.15, 0.18).scale(conditionFactor);
				break;
			case "racing":
				frameMat.albedoColor = new Color3(0.8, 0.1, 0.1).scale(conditionFactor);
				break;
			case "mamachari":
				frameMat.albedoColor = new Color3(0.5, 0.45, 0.35).scale(conditionFactor);
				break;
			case "folding":
				frameMat.albedoColor = new Color3(0.85, 0.5, 0.1).scale(conditionFactor);
				break;
			case "vintage":
				frameMat.albedoColor = new Color3(0.15, 0.3, 0.25).scale(conditionFactor);
				break;
		}
		...
		const wheelRadius = type === "folding" ? 0.2 : type === "racing" ? 0.35 : 0.3;
		...
		for (const side of [-1, 1]) {
			...
			grip.position = new Vector3(
				posX + Math.cos(rotation) * (wheelbase / 2) + Math.sin(rotation) * (side * handlebarWidth / 2),
				baseY + wheelRadius + frameHeight + 0.1,
				posZ - Math.sin(rotation) * (wheelbase / 2) + Math.cos(rotation) * (side * handlebarWidth / 2)
			);
		...
		if (hasBasket || type === "mamachari") {
			...
			basketMat.albedoColor = new Color3(0.5, 0.52, 0.55).scale(conditionFactor);
	...
	}, [scene, id, posX, posY, posZ, type, state, hasBasket, condition, rotation, seed]);
