	...
	useEffect(() => {
		...
		const getColor = (colorName: BuoyColor): Color3 => {
			switch (colorName) {
				...
				case "green": return new Color3(0.1, 0.6, 0.2);
				case "yellow": return new Color3(0.9, 0.85, 0.1);
		...
		if (type === "marker" || type === "channel") {
			...
			if (type === "channel") {
				...
				stripeMat.albedoColor = color === "red" ? new Color3(0.95, 0.95, 0.95) : new Color3(0.1, 0.1, 0.12);
				stripeMat.metallic = 0.2;
				stripeMat.roughness = 0.6;

				const stripe = MeshBuilder.CreateTorus(
					`${id}_stripe`,
					{ diameter: baseRadius * 1.4, thickness: baseHeight * 0.15, tessellation: 16 },
					scene
				);
			...
			const eye = MeshBuilder.CreateTorus(
				`${id}_eye`,
				{ diameter: baseRadius * 0.6, thickness: baseRadius * 0.1, tessellation: 12 },
				scene
			);
			...
			const tower = MeshBuilder.CreateCylinder(
				`${id}_tower`,
				{ height: baseHeight * 1.5, diameter: baseRadius * 0.8, tessellation: 12 },
				scene
			);
		...
		if (hasLight || type === "light") {
			...
			const lightY = type === "light"
				? posY + 0.4 + baseHeight * 1.5 + 0.35
				: posY + baseHeight + 0.15;
