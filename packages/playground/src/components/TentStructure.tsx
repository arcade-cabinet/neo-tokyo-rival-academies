...
export function TentStructure({
	...
	useEffect(() => {
		...
		const getColor = (colorName: string): Color3 => {
			switch (colorName) {
				case "blue": return new Color3(0.2, 0.35, 0.55);
				case "green": return new Color3(0.25, 0.4, 0.25);
				case "tan": return new Color3(0.6, 0.55, 0.4);
				case "orange": return new Color3(0.85, 0.45, 0.15);
				case "gray": return new Color3(0.45, 0.47, 0.5);
				case "camo": return new Color3(0.35, 0.38, 0.3);
				default: return new Color3(0.2, 0.35, 0.55);
			}
		};
		...
		const stateTilt = state === "weathered" ? (rng ? rng.next() * 0.1 : 0.05) : 0;
		...
		if (type === "camping") {
			...
			for (const end of [-1, 1]) {
				const endPanel = MeshBuilder.CreateCylinder(
					`${id}_end_${end}`,
					{ height: 0.02, diameterTop: 0, diameterBottom: tentWidth * 1.4, tessellation: 3 },
					scene
				);
			...
			for (let x = -1; x <= 1; x += 2) {
				for (let z = -1; z <= 1; z += 2) {
					...
					pole.position = new Vector3(
						posX + Math.cos(rotation) * (x * canopyWidth * 0.45) - Math.sin(rotation) * (z * canopyDepth * 0.45),
						posY + canopyHeight / 2,
						posZ - Math.sin(rotation) * (x * canopyWidth * 0.45) - Math.cos(rotation) * (z * canopyDepth * 0.45)
					);
			...
			for (let p = 0; p < poleCount; p++) {
				const px = (rng ? rng.next() - 0.5 : p / poleCount - 0.5) * tarpWidth * 0.8;
				const pz = (rng ? rng.next() - 0.5 : 0) * tarpLength * 0.8;
				const pHeight = tarpHeight * (0.7 + (rng ? rng.next() * 0.5 : 0.25));

				const pole = MeshBuilder.CreateCylinder(
					`${id}_pole_${p}`,
					{ height: pHeight, diameter: 0.03 + (rng ? rng.next() * 0.02 : 0.01) },
					scene
				);
				pole.position = new Vector3(
	...
	}, [scene, id, posX, posY, posZ, type, state, size, color, isLit, hasStakes, condition, rotation, seed]);
