...
export function Elevator({
	...
	useEffect(() => {
		...
		if (type === "external") {
			...
			for (let i = 0; i < framePosts.length; i++) {
				...
				for (const dir of [-1, 1]) {
					...
					flange.position = new Vector3(
						posX + Math.cos(rotation) * fx - Math.sin(rotation) * (fz + dir * 0.075),
						posY + height / 2,
						posZ + Math.sin(rotation) * fx + Math.cos(rotation) * (fz + dir * 0.075)
					);
			...
			for (const side of [-1, 1]) {
				...
				rail.position = new Vector3(
					posX + Math.cos(rotation) * (side * (width / 2 - 0.15)) - Math.sin(rotation) * (depth / 2 - 0.1),
					posY + height / 2,
					posZ - Math.sin(rotation) * (side * (width / 2 - 0.15)) - Math.cos(rotation) * (depth / 2 - 0.1)
				);
				rail.rotation.y = rotation;
		...
		if (hasMotorRoom) {
			...
			const motorRoom = MeshBuilder.CreateBox(
				`${id}_motor_room`,
				{ width: motorRoomWidth, height: motorRoomHeight, depth: motorRoomDepth },
				scene
			);
			motorRoom.position = new Vector3(posX, posY + height + motorRoomHeight / 2, posZ);
			...
			for (const side of [-1, 1]) {
				const louver = MeshBuilder.CreateBox(
					...
					{ width: 0.02, height: motorRoomHeight * 0.5, depth: motorRoomDepth * 0.6 },
					scene
				);
				louver.position = new Vector3(
					posX + Math.cos(rotation) * (side * motorRoomWidth / 2 + side * 0.01),
					posY + height + motorRoomHeight * 0.5,
					posZ - Math.sin(rotation) * (side * motorRoomWidth / 2 + side * 0.01)
				);
	...
	}, [scene, id, posX, posY, posZ, type, rotation, condition, seed, height, width, hasMotorRoom, currentFloor]);
