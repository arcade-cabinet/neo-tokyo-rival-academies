...
export function Bridge({
			...
			for (const cableSide of [-1, 1]) {
				for (let s = 0; s < cableSegments; s++) {
					...
					cable.position = new Vector3(
						posX + Math.cos(rotation) * ((x1 + x2) / 2) - Math.sin(rotation) * (cableSide * width / 2),
						posY + (y1 + y2) / 2,
						posZ - Math.sin(rotation) * ((x1 + x2) / 2) - Math.cos(rotation) * (cableSide * width / 2)
					);
	...
	}, [scene, id, posX, posY, posZ, type, length, width, clearance, hasRailings, hasLights, rotation, condition, seed]);
