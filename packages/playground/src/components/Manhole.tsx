		...
		if (type === "round") {
			...
			if (state === "closed") {
				...
				for (let i = 1; i <= 3; i++) {
					...
					ring.position = new Vector3(posX, posY + coverThickness + 0.002, posZ);
				...
				if (rng) {
					...
					for (let d = 0; d < debrisCount; d++) {
						const debrisMat = new PBRMaterial(`manhole_debris_${id}_${d}`, scene);
			...
			if (state === "closed") {
			...
			} else if (state === "open" || state === "missing" || state === "flooded") {
				// Hole
				...
				holeBox.material = state === "flooded" ? (() => {
					const waterMat = new PBRMaterial(`manhole_water_${id}`, scene);
					waterMat.albedoColor = new Color3(0.15, 0.25, 0.35);
					waterMat.metallic = 0.3;
					waterMat.roughness = 0.1;
			...
			const frame = MeshBuilder.CreateBox(
				`${id}_frame`,
				{ width: coverWidth + 0.04, height: frameThickness, depth: coverDepth + 0.04 },
				scene
			);
			frame.position = new Vector3(posX, posY + frameThickness / 2, posZ);
			...
			if (state === "closed" || state === "flooded") {
				...
				cover.position = new Vector3(posX, posY + frameThickness + coverThickness / 2, posZ);
				cover.rotation.y = rotation;
				cover.material = coverMat;
				meshes.push(cover);
				...
				label.position = new Vector3(posX, posY + frameThickness + coverThickness + 0.001, posZ);
			...
			const frame = MeshBuilder.CreateTorus(
				`${id}_frame`,
				{ diameter: coverDiameterDeco + frameThickness, thickness: frameThickness },
				scene
			);
			frame.position = new Vector3(posX, posY + frameThickness / 2, posZ);
			frame.rotation.x = Math.PI / 2;
			...
			if (state === "closed") {
				...
				centralDesign.position = new Vector3(posX, posY + coverThickness + 0.002, posZ);
				...
				outerRing.position = new Vector3(posX, posY + coverThickness + 0.003, posZ);
				...
				textRing.position = new Vector3(posX, posY + coverThickness + 0.003, posZ);
				...
				holeDisc.position = new Vector3(posX, posY + 0.001, posZ);
				holeDisc.rotation.x = Math.PI / 2;
				holeDisc.material = state === "flooded" ? (() => {
					const waterMat = new PBRMaterial(`manhole_water_${id}`, scene);
					waterMat.albedoColor = new Color3(0.15, 0.25, 0.35);
					waterMat.metallic = 0.3;
					waterMat.roughness = 0.1;
