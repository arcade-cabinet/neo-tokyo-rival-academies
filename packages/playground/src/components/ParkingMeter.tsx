...
export function ParkingMeter({
	...
	useEffect(() => {
		...
		if (type === "single") {
			...
			for (const side of [-1, 1]) {
				...
				const indicatorMat = new PBRMaterial(`meter_indicator_${id}_${side}`, scene);
			...
			for (const [dx, dz, w, d] of [
				...
				framePiece.position = new Vector3(
					posX - Math.sin(rotation) * 0.02 + Math.cos(rotation) * dx - Math.sin(rotation) * dz * Math.cos(0.4),
					posY + poleHeight + kioskHeight + 0.05 + dz * Math.sin(0.4),
					posZ - Math.cos(rotation) * 0.02 - Math.sin(rotation) * dx - Math.cos(rotation) * dz * Math.cos(0.4)
				);
		...
		if (isBroken && rng) {
			...
			for (let i = 0; i < crackCount; i++) {
				...
				crack.position = new Vector3(
					...
					posY + poleHeight + meterHeight / 2 + yOffset + (rng.next() - 0.5) * 0.08,
	...
	}, [scene, id, posX, posY, posZ, type, isExpired, isBroken, condition, rotation, seed]);
