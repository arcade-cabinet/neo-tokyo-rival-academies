...
const CART_DIMENSIONS: Record<ShoppingCartType, { width: number; height: number; depth: number; wheelSize: number }> = {
	...
	useEffect(() => {
		...
		if (type === "standard" || type === "small") {
			...
			for (const side of [-1, 1]) {
				...
				sidePanel.position = transformPos(side * (width / 2 - 0.03), basketBottom + basketHeight / 2, -0.05);
			...
			for (const side of [-1, 1]) {
				...
				support.position = transformPos(side * (width / 2 - 0.1), platformHeight + height / 2, -depth / 2 - 0.05);
			...
			for (const side of [-1, 1]) {
				...
				post.position = transformPos(side * (width / 2 - 0.08), platformHeight + handleFrameHeight / 2, -depth / 2);
		...
		if (state === "abandoned" && rng) {
			...
			const basketBottom = type === "warehouse" || type === "airport"
			...
			for (let i = 0; i < debrisCount; i++) {
				...
				const debris = MeshBuilder.CreateBox(
					`${id}_debris_${i}`,
					{ width: debrisSize, height: debrisSize * 0.5, depth: debrisSize * 1.2 },
					scene
				);
