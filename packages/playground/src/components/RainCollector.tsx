...
export function RainCollector({
	...
	useEffect(() => {
		...
		metalMat.albedoColor = new Color3(0.4 * ageFactor, 0.42 * ageFactor, 0.45 * ageFactor);
		...
		if (type === "barrel") {
			...
			barrelMat.albedoColor = new Color3(0.15 * ageFactor, 0.2 * ageFactor, 0.5 * ageFactor);
			...
			tankMat.albedoColor = new Color3(0.2 * ageFactor, 0.22 * ageFactor, 0.25 * ageFactor);
			...
			bucketMat.albedoColor = new Color3(0.5 * ageFactor, 0.5 * ageFactor, 0.52 * ageFactor);
			...
			handle.position = new Vector3(posX, posY + bucketHeight + bucketTopDiameter * 0.35, posZ);
			...
			if (fillLevel > 0) {
				...
				const waterDiameter = bucketBottomDiameter + (bucketTopDiameter - bucketBottomDiameter) * fillLevel * 0.85;
				const water = MeshBuilder.CreateCylinder(
					`${id}_water`,
					{
						height: waterHeight,
			...
			tarpMat.albedoColor = new Color3(0.15 * ageFactor, 0.35 * ageFactor, 0.2 * ageFactor);
			tarpMat.metallic = 0;
			...
			if (fillLevel > 0) {
				...
				water.position = new Vector3(posX, posY + 0.32 - tarpSag + waterDepth / 2, posZ);
				water.material = waterMat;
				meshes.push(water);
			}
		...
		if (condition === "damaged" && rng) {
			...
			const baseHeight = type === "barrel" ? 0.9 * scale : type === "tank" ? 1.2 * scale : 0.35 * scale;
			...
			for (let d = 0; d < damageCount; d++) {
				...
				const radius = type === "barrel" ? 0.55 * scale / 2 : type === "bucket" ? 0.35 * scale / 2 : 0.4 * scale;
	...
	}, [scene, id, posX, posY, posZ, rotation, type, condition, capacity, fillLevel, hasSpout, seed]);
