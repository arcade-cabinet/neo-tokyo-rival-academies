		...
		if (type === "portable") {
			...
			for (let o = 0; o < 2; o++) {
				...
				outlet.position = new Vector3(
					posX + Math.sin(rotation) * (bodyDepth / 2 + 0.02) + Math.cos(rotation) * ((o - 0.5) * 0.12),
					posY + bodyHeight * 0.35,
					posZ + Math.cos(rotation) * (bodyDepth / 2 + 0.02) - Math.sin(rotation) * ((o - 0.5) * 0.12)
				);
			...
			cabinet.position = new Vector3(
				posX + Math.cos(rotation) * (-bodyWidth / 2 - 0.35) + Math.sin(rotation) * (bodyDepth / 2 - 0.3),
				posY + 0.6,
				posZ - Math.sin(rotation) * (-bodyWidth / 2 - 0.35) + Math.cos(rotation) * (bodyDepth / 2 - 0.3)
			);
