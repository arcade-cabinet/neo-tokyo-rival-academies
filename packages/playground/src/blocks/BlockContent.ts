...
import type { BlockDefinition, BlockCategory, FactionAffinity } from "./Block";
...
export interface BlockContentDefinition {
	...
	factionVariants?: Partial<Record<FactionAffinity, {
		props?: ComponentSpawnRule[];
		decoration?: ComponentSpawnRule[];
		colorScheme?: {
			primary: Color3;
...
export const CONTENT_RTB_SHELTER: BlockContentDefinition = {
	...
	structure: {
		...
		walls: [
			{ side: "north", component: "TexturedWall", props: { textureType: "metal_corrugated" } },
			{ side: "east", component: "TexturedWall", props: { textureType: "metal_corrugated" } },
			{ side: "west", component: "TexturedWall", props: { textureType: "metal_corrugated" } },
 ...
 */
export const BLOCK_CONTENT_REGISTRY: Map<BlockCategory, BlockContentDefinition[]> = new Map();
...
export function getBlockContent(
	...
	const specific = categoryContents.find((c) => c.blockTypeId === definition.typeId);
...
export function registerAllBlockContent(): void {
	...
	console.log("[BlockContent] Registered content for", BLOCK_CONTENT_REGISTRY.size, "categories");
}
	...
	const generatePosition = (index: number): Vector3 => {
		...
		switch (rule.placement) {
			...
			case "perimeter": {
				...
				const t = rng.next();
				if (edge === 0) { x = t * blockDimensions.width - halfW; z = -halfD + 0.5; }
				else if (edge === 1) { x = t * blockDimensions.width - halfW; z = halfD - 0.5; }
				else if (edge === 2) { x = -halfW + 0.5; z = t * blockDimensions.depth - halfD; }
				else { x = halfW - 0.5; z = t * blockDimensions.depth - halfD; }
				break;
			}
