...
// Import ALL components that can be spawned
// Structure
...
function StructureRenderer({
	...
	if (content.structure.walls) {
		for (const wall of content.structure.walls) {
			...
			let wallSize = { width: 0, height, depth: 0.2 };
...
export function BlockContentRenderer({
	...
	if (debug) {
		...
		spawns.forEach((s, i) => {
			console.log(`  [${i}] ${s.component} at (${s.position.x.toFixed(1)}, ${s.position.y.toFixed(1)}, ${s.position.z.toFixed(1)})`);
		});
	}
