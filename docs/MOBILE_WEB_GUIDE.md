...
- **Vite** via Angular build tooling

## Repository Layout (Single App)
```
root/
├── app/                 # Ionic Angular application (single unified app)
...
1. Build the web app: `pnpm -C app build`
2. Sync native platforms: `pnpm -C app cap:sync`
3. Open platforms:
   - Android: `pnpm -C app cap:open:android`
   - iOS: `pnpm -C app cap:open:ios`

## Babylon.js Mobile Notes
- Prefer **meshToonMaterial** (or Babylon ToonMaterial equivalent) for the cel-shaded look.
...
- HUD overlays should use `position: absolute` and pointer lock only where necessary.
